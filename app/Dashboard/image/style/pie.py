...
def pie_style(theme, custom_theme_colors, custom_bg_color, bg_color_rgb, pie, pie_radius, pie_outer_radius,
    ...
    if pie:
        browser.find_element_by_xpath(
            "//*[@name='pie-style']/following-sibling::span[text()='{0}']".format(pie)).click()
        log.info("设置饼图样式: {0}".format(pie))
        sleep(1)

    # 半径
    if pie_radius:
        slider = browser.find_element_by_xpath(tab_div_xpath + "//*[@id='pie-radius']")
        basic_value = slider.get_attribute("value")
        x_offset = (int(pie_radius) - int(basic_value)) * 2
        size_slider = browser.find_element_by_xpath(
            "//*[@id='pie-radius']/following-sibling::div[1]//a")
    ...
    # 外半径
    if pie_outer_radius:
        slider = browser.find_element_by_xpath(tab_div_xpath + "//*[@id='pie-outer-radius']")
        basic_value = slider.get_attribute("value")
        x_offset = (int(pie_outer_radius) - int(basic_value)) * 2
        size_slider = browser.find_element_by_xpath(
            "//*[@id='pie-outer-radius']/following-sibling::div[1]//a")
        action.drag_and_drop_by_offset(size_slider, x_offset, 0).perform()
    ...
    # 内半径
    if pie_inner_radius:
        slider = browser.find_element_by_xpath(tab_div_xpath + "//*[@id='pie-inner-radius']")
        basic_value = slider.get_attribute("value")
        x_offset = (int(pie_inner_radius) - int(basic_value)) * 2
        size_slider = browser.find_element_by_xpath(
            "//*[@id='pie-inner-radius']/following-sibling::div[1]//a")
        action.drag_and_drop_by_offset(size_slider, x_offset, 0).perform()
    ...
    if show_legend:
        browser.find_element_by_xpath(
            "//*[@name='legend-visible']/following-sibling::span[text()='{0}']".format(show_legend)).click()
    ...
    if legend_direction:
        browser.find_element_by_xpath(
            "//*[@name='legend-direction']/following-sibling::span[text()='{0}']".format(legend_direction)).click()
    ...
    if legend_align:
        browser.find_element_by_xpath(
            "//*[@name='legend-align']/following-sibling::span[text()='{0}']".format(legend_align)).click()
        log.info("设置图例对齐方式: {0}".format(legend_align))
        sleep(1)

    # 图例字体大小
    if legend_font_size:
        slider = browser.find_element_by_xpath(tab_div_xpath + "//*[@id='legendFontSize']")
        basic_value = slider.get_attribute("value")
        x_offset = (int(legend_font_size) - int(basic_value)) * 5
        size_slider = browser.find_element_by_xpath(
            "//*[@id='legendFontSize']/following-sibling::div[1]//a")
        action.drag_and_drop_by_offset(size_slider, x_offset, 0).perform()
    ...
    if legend_sortable:
        ...
        if tmp ^ status:
            browser.find_element_by_xpath(tab_div_xpath + "//*[@id='legendSortable']").click()
