...
def table_style(theme, custom_bg_color, bg_color_rgb, show_title, title_alignment, title_font_size, page_size,
    ...
    # 每页展示条数
    if page_size:
        browser.find_element_by_xpath("//*[text()='每页展示条数']/following-sibling::span//a").click()
        ps_elements = browser.find_elements_by_xpath(
            "//*[contains(@id,'combobox') and text()='{0}']".format(page_size))
    ...
    if col_align:
        browser.find_element_by_xpath(
            tab_div_xpath + "//*[@name='col-align']/following-sibling::span[text()='{0}']".format(
                col_align)).click()
    ...
    if col_width:
        browser.find_element_by_xpath(
            tab_div_xpath + "//*[@name='col-width']/following-sibling::span[text()='{0}']".format(
                col_width)).click()
    ...
    # 冻结列
    if frozen_column:
        browser.find_element_by_xpath("//*[text()='冻结列']/following-sibling::span//a").click()
        fc_elements = browser.find_elements_by_xpath(
            "//*[contains(@id,'combobox') and text()='{0}']".format(frozen_column))
