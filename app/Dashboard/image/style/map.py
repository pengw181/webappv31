...
def map_style(theme, custom_bg_color, bg_color_rgb, show_title, title_alignment, title_font_size, show_district,
    ...
    if show_district:
        browser.find_element_by_xpath(
            tab_div_xpath + "//*[@name='show-label']/following-sibling::span[text()='{0}']".format(
                show_district)).click()
    ...
    if show_color_bar:
        browser.find_element_by_xpath(
            tab_div_xpath + "//*[@name='show-visualMap']/following-sibling::span[text()='{0}']".format(
    ...
    # 最小值
    if min_value:
        browser.find_element_by_xpath(tab_div_xpath + "//*[@name='min']").clear()
        browser.find_element_by_xpath(tab_div_xpath + "//*[@name='min']").send_keys(min_value)
        log.info("设置最小值: {0}".format(min_value))
        sleep(1)

    # 最大值
    if max_value:
        browser.find_element_by_xpath(tab_div_xpath + "//*[@name='max']").clear()
        browser.find_element_by_xpath(tab_div_xpath + "//*[@name='max']").send_keys(max_value)
        log.info("设置最大值: {0}".format(max_value))
        sleep(1)

    # 下级最小值
    if min_value1:
        browser.find_element_by_xpath(tab_div_xpath + "//*[@name='min1']").clear()
        browser.find_element_by_xpath(tab_div_xpath + "//*[@name='min1']").send_keys(min_value1)
        log.info("设置下级最小值: {0}".format(min_value1))
        sleep(1)

    # 下级最大值
    if max_value1:
        browser.find_element_by_xpath(tab_div_xpath + "//*[@name='max1']").clear()
        browser.find_element_by_xpath(tab_div_xpath + "//*[@name='max1']").send_keys(max_value1)
