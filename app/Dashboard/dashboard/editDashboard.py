...
class EditDashboard:
    ...
    def edit(self, dashboard_name, subtitle, remark, theme, show_title, carousel, carousel_interval):
        ...
        if dashboard_name:
            self.browser.find_element_by_xpath(
                self.xpath + "//*[@name='dashboard-name']/preceding-sibling::input[1]").clear()
            self.browser.find_element_by_xpath(
                self.xpath + "//*[@name='dashboard-name']/preceding-sibling::input[1]").send_keys(dashboard_name)
            log.info("设置仪表盘名称: {0}".format(dashboard_name))

        # 仪表盘副标题
        if subtitle:
            self.browser.find_element_by_xpath(
                self.xpath + "//*[@name='dashboard-sub-name']/preceding-sibling::input[1]").clear()
            self.browser.find_element_by_xpath(
                self.xpath + "//*[@name='dashboard-sub-name']/preceding-sibling::input[1]").send_keys(subtitle)
            log.info("设置仪表盘副标题: {0}".format(subtitle))

        # 备注
        if remark:
            self.browser.find_element_by_xpath(
                self.xpath + "//*[@name='dashboard-remark']/preceding-sibling::input[1]").clear()
            self.browser.find_element_by_xpath(
                self.xpath + "//*[@name='dashboard-remark']/preceding-sibling::input[1]").send_keys(remark)
            log.info("设置备注: {0}".format(remark))

        # 主题样式
        if theme:
            self.browser.find_element_by_xpath(self.xpath + "//*[text()='主题样式']/following-sibling::div[1]/div").click()
            self.browser.find_element_by_xpath(self.xpath + "//*[text()='{0}']".format(theme)).click()
        ...
        if show_title:
            ...
            show_title_element = self.browser.find_element_by_xpath(self.xpath + "//*[@name='show-title']")
            if tmp ^ show_status:
        ...
        if carousel:
            ...
            carousel_element = self.browser.find_element_by_xpath(self.xpath + "//*[@name='carousel-auto']")
        ...
        if carousel_interval:
            self.browser.find_element_by_xpath(
                self.xpath + "//*[@name='dashboard-carousel-interval']/preceding-sibling::input[1]").clear()
            self.browser.find_element_by_xpath(
                self.xpath + "//*[@name='dashboard-carousel-interval']/preceding-sibling::input[1]").send_keys(
                carousel_interval)
            log.info("设置轮播间隔: {0}".format(carousel_interval))

        # 保存
        self.browser.find_element_by_xpath(self.xpath + "//*[text()='保存']").click()
    ...
    def addInImage(self, image_list):
        ...
        """
        self.browser.find_element_by_xpath(self.xpath + "//*[text()='添加图像']").click()
        page_wait(5)
        wait = WebDriverWait(self.browser, 5)
        wait.until(ec.element_to_be_clickable((By.XPATH, "//*[@name='visualImageName']/preceding-sibling::input[1]")))
        for image in image_list:
            self.browser.find_element_by_xpath(
                "//*[@id='selectImageDlg']//*[@field='visualImageName']//*[@data-mtips='{0}']".format(image)).click()
            log.info("已选择图像: {0}".format(image))
        self.browser.find_element_by_xpath("//*[@id='selectImageDlg']//*[text()='确定']").click()
        log.info("加入图像成功")
        # 保存
        self.browser.find_element_by_xpath(self.xpath + "//*[text()='保存']").click()
        set_global_var("ResultMsg", "操作成功", False)
...
class MyDashboard(EditDashboard):
    ...
    def __init__(self):
        super().__init__()
        self.browser.find_element_by_xpath("//*[text()='我的仪表盘']").click()
        page_wait(3)
...
class DefaultDashboard(EditDashboard):
    ...
    def __init__(self):
        super().__init__()
        self.browser.find_element_by_xpath("//*[text()='默认仪表盘']").click()
        page_wait(3)
