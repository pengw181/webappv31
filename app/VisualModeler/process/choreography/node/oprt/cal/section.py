...
def section(output_var, input_var, value_type, begin_config, end_config, sample_data):
    ...
    # 切换到分段拆分运算iframe
    browser.switch_to.frame(browser.find_element_by_xpath("//iframe[contains(@src,'operateCfgSection.html')]"))
    ...
    # 输入样例数据
    if sample_data:
        textarea = browser.find_element_by_xpath("//*[@id='section_area']//textarea[@id='sampleData']")
    ...
    # 格式化结果
    format_ele = browser.find_element_by_xpath("//*[@onclick=\"previewSection('1')\"]//*[text()='格式化结果']")
...
def begin_sep_set(enable_flag, set_type, regular_name, advance_mode, regular, expression):
    ...
    # 聚焦元素
    begin_sep = browser.find_element_by_xpath("//*[@id='isBeginRow1']")
    ...
    if enable_flag:
        ...
        regular_cube(confirm_selector=confirm_selector, set_type=set_type, regular_name=regular_name,
        ...
        if set_type == "添加":
            ...
            if alert.title_contains("成功"):
                ...
                browser.switch_to.frame(
                    browser.find_element_by_xpath("//iframe[contains(@src,'operateCfgSection.html')]"))
            else:
...
def end_sep_set(enable_flag, set_type, regular_name, advance_mode, regular, expression):
    ...
    # 聚焦元素
    end_sep = browser.find_element_by_xpath("//*[@id='isBeginRow2']")
