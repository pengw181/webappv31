...
class NodeControl:
    ...
    def condition_dependence(self, enable):
        ...
        if enable:
            ...
            if temp ^ status:
                self.browser.find_element_by_xpath("//*[@id='is_cnd_rely']").click()
    ...
    def set_loop(self, enable, loopTagList, loopType, loopContent):
        ...
        if enable:
            ...
            temp = True if enable == "开启" else False
            if temp ^ status:
                self.browser.find_element_by_xpath("//*[@id='_node_circle_tag']").click()
                sleep(1)
        ...
        if loopTagList:
            # 先将已选择的标签取消勾选
            try:
                self.browser.find_element_by_xpath("//*[@id='loopchecktr']//*[text()='业务配置']")
                # 获取业务配置勾选状态
                js = 'return $("#is_busi_loop")[0].checked;'
                status = self.browser.execute_script(js)
                log.info("循环条件【业务配置】勾选状态: {0}".format(status))
                if status:
                    self.browser.find_element_by_xpath("//*[@id='is_busi_loop']").click()
            except NoSuchElementException:
                pass

            try:
                self.browser.find_element_by_xpath("//*[@id='loopchecktr']//*[text()='取数配置']")
                # 获取取数配置勾选状态
                js = 'return $("#is_getdata_loop")[0].checked;'
                status = self.browser.execute_script(js)
                log.info("循环条件【取数配置】勾选状态: {0}".format(status))
                if status:
                    self.browser.find_element_by_xpath("//*[@id='is_getdata_loop']").click()
            except NoSuchElementException:
                pass

            try:
                self.browser.find_element_by_xpath("//*[@id='loopchecktr']//*[text()='操作配置']")
                # 获取操作配置勾选状态
                js = 'return $("#is_operate_loop")[0].checked;'
                status = self.browser.execute_script(js)
                log.info("循环条件【操作配置】勾选状态: {0}".format(status))
                if status:
                    self.browser.find_element_by_xpath("//*[@id='is_operate_loop']").click()
            except NoSuchElementException:
                pass

            # 依次勾选循环条件
            for tag in loopTagList:
                self.browser.find_element_by_xpath("//*[@id='loopchecktr']//*[text()='{0}']".format(tag)).click()
                log.info("循环条件勾选【{0}】".format(tag))

        # 循环类型
        if loopType:
            self.browser.find_element_by_xpath(
                "//*[@name='looptype']/following-sibling::span[text()='{0}']".format(loopType)).click()
    ...
    def _var_list_loop(self, mode, varName, outVarName, valueType, varType):
        ...
        if mode:
            self.browser.find_element_by_xpath(
                "//*[@name='mode']/following-sibling::label[text()='{0}']".format(mode)).click()
            log.info("选择模式: {0}".format(mode))
            sleep(1)

        # 变量类型
        if varType:
            self.browser.find_element_by_xpath("//*[@id='vartype']/following-sibling::span//a").click()
            self.browser.find_element_by_xpath("//*[contains(@id,'vartype') and text()='{0}']".format(varType)).click()
            log.info("选择变量类型: {0}".format(varType))

        # 变量名称
        if varName:
            self.browser.find_element_by_xpath(
                "//*[@id='dataH_inputVarControl2Name']/following-sibling::span//a").click()
            choose_var(var_name=varName)

        # 循环行变量名称，选择变量后会自动填充
        if outVarName:
            self.browser.find_element_by_xpath("//*[@id='outVarName']/following-sibling::span//input[1]").clear()
            self.browser.find_element_by_xpath(
                "//*[@id='outVarName']/following-sibling::span//input[1]").send_keys(outVarName)
            log.info("设置循环行变量名称: {0}".format(outVarName))

        # 赋值方式
        if valueType:
            elements = self.browser.find_elements_by_xpath("//*[@id='valuetype_1']/following-sibling::span[1]//a")
            for e1 in elements:
                if e1.is_displayed():
                    e1.click()
                    elements = self.browser.find_elements_by_xpath(
                        "//*[contains(@id,'valueType') and text()='{0}']".format(valueType))
                    for e2 in elements:
                        if e2.is_displayed():
                            e2.click()
    ...
    def _circle_loop(self, circleTimes, outVarName, valueType, toNextCondition, endCondition):
        ...
        if circleTimes:
            ...
            form_selector = "//*[@id='con_ctrl_cfg']"
            set_text_enable_var(input_xpath=input_xpath, form_selector=form_selector, msg=circleTimes)
            log.info("设置循环次数: {0}".format(circleTimes))

        # 循环变量名称
        if outVarName:
            self.browser.find_element_by_xpath("//*[@id='outVarName_2']/following-sibling::span/input[1]").clear()
            self.browser.find_element_by_xpath(
                "//*[@id='outVarName_2']/following-sibling::span/input[1]").send_keys(outVarName)
            log.info("设置循环变量名称: {0}".format(outVarName))

        # 赋值方式
        if valueType:
            elements = self.browser.find_elements_by_xpath("//*[@id='valuetype_2']/following-sibling::span[1]//a")
            for e1 in elements:
                if e1.is_displayed():
                    e1.click()
                    elements = self.browser.find_elements_by_xpath(
                        "//*[contains(@id,'valuetype_2') and text()='{0}']".format(valueType))
                    for e2 in elements:
                        if e2.is_displayed():
                            e2.click()
                            log.info("设置赋值方式: {0}".format(valueType))
        ...
        if toNextCondition:
            # 先清空
            self.browser.find_element_by_xpath(
                "//*[contains(@onclick,'deleteAdd') and contains(@onclick,'times_nextCondition')]").click()
            # 点击修改
            self.browser.find_element_by_xpath(
                "//*[contains(@onclick,'showAdd') and contains(@onclick,'times_nextCondition')]").click()
            page_wait()
        ...
        if endCondition:
            # 先清空
            self.browser.find_element_by_xpath(
                "//*[contains(@onclick,'deleteAdd') and contains(@onclick,'times_endCondition')]").click()
            # 点击修改
            self.browser.find_element_by_xpath(
                "//*[contains(@onclick,'showAdd') and contains(@onclick,'times_endCondition')]").click()
            page_wait()
            iframe_xpath_list = [get_global_var("NodeIframe"), get_global_var("ControlIframe")]
    ...
    def _condition_loop(self, circleCondition, toNextCondition, endCondition):
        ...
        if circleCondition:
            # 先清空
            self.browser.find_element_by_xpath(
                "//*[contains(@onclick,'deleteAdd') and contains(@onclick,'circleCondition')]").click()
            # 点击修改
            self.browser.find_element_by_xpath(
                "//*[contains(@onclick,'showAdd') and contains(@onclick,'circleCondition')]").click()
            page_wait()
            iframe_xpath_list = [get_global_var("NodeIframe"), get_global_var("ControlIframe")]
        ...
        if toNextCondition:
            # 先清空
            del_elements = self.browser.find_elements_by_xpath(
                "//*[contains(@onclick,'deleteAdd') and contains(@onclick,'nextCondition')]")
            ...
            for element in del_elements:
                if element.is_displayed():
                    ...
                    break
            # 点击修改
            show_elements = self.browser.find_elements_by_xpath(
                "//*[contains(@onclick,'showAdd') and contains(@onclick,'nextCondition')]")
        ...
        if endCondition:
            # 先清空
            del_elements = self.browser.find_elements_by_xpath(
                "//*[contains(@onclick,'deleteAdd') and contains(@onclick,'endCondition')]")
            if len(del_elements) == 0:
                raise NoSuchElementException
            for element in del_elements:
                if element.is_displayed():
            ...
            # 点击修改
            show_elements = self.browser.find_elements_by_xpath(
                "//*[contains(@onclick,'showAdd') and contains(@onclick,'endCondition')]")
            if len(show_elements) == 0:
                raise NoSuchElementException
            for element in show_elements:
                if element.is_displayed():
    ...
    def get_value_by_col(self, enable, varSetList):
        ...
        if enable:
            ...
            if not (temp and status):
                self.browser.find_element_by_xpath("//*[@id='is_getValueByCol']").click()
                sleep(1)
        ...
        if varSetList:
            ...
            for var_info in varSetList:
                ...
                if opt_type == "添加":
                    # 点击添加
                    self.browser.find_element_by_xpath("//*[@onclick='addCtrlGetInfo();']").click()
                    ...
                    self.browser.find_element_by_xpath("//*[@field='varName']/*[text()='{0}']".format(obj_var)).click()

                    if opt_type == "修改":
                        # 点击修改
                        self.browser.find_element_by_xpath("//*[@onclick='edit_ctrcfg_var();']").click()
                        self._col_var_page(varName=var_name, valueType=value_type, arrayIndex=array_index)
                    else:
                        # 点击删除
                        self.browser.find_element_by_xpath("//*[@onclick='delete_ctrcfg_var();']").click()
    ...
    def _col_var_page(self, varName, valueType, arrayIndex):
        ...
        # 变量名称
        if varName:
            self.browser.find_element_by_xpath("//*[@id='ctrlvarName']/following-sibling::span/input[1]").clear()
            self.browser.find_element_by_xpath(
                "//*[@id='ctrlvarName']/following-sibling::span/input[1]").send_keys(varName)
            log.info("【按列取数】设置变量名称: {0}".format(varName))

        # 赋值方式
        if valueType:
            elements = self.browser.find_elements_by_xpath("//*[@id='valuetype']/following-sibling::span[1]//a")
            for e1 in elements:
                if e1.is_displayed():
                    e1.click()
                    elements = self.browser.find_elements_by_xpath(
        ...
        if arrayIndex:
            self.browser.find_element_by_xpath(
                "//*[@id='ctrlarrayIndex']/following-sibling::span/input[1]").clear()
            self.browser.find_element_by_xpath(
                "//*[@id='ctrlarrayIndex']/following-sibling::span/input[1]").send_keys(arrayIndex)
            log.info("【按列取数】设置数据索引: {0}".format(arrayIndex))

        # 保存
        self.browser.find_element_by_xpath("//*[@onclick='addColumnVar()']").click()
        alert = BeAlertBox(back_iframe="default")
        msg = alert.get_msg()
        if alert.title_contains("操作成功"):
            log.info("【按列取数】设置变量成功")
    ...
    def advance_cfg(self, saveLog, recordCycleNum, outputLogPrintRuler):
        ...
        """
        # 展开高级配置
        advance_element = self.browser.find_element_by_xpath("//*[@id='adv_btn']")
        ...
        if saveLog:
            ...
            if temp ^ status:
                self.browser.find_element_by_xpath("//*[@id='is_addLog']").click()
                sleep(1)
        ...
        # 循环日志记录条数
        if recordCycleNum:
            self.browser.find_element_by_xpath("//*[@id='recordCycleNum']/following-sibling::span").clear()
            self.browser.find_element_by_xpath(
                "//*[@id='recordCycleNum']/following-sibling::span").send_keys(recordCycleNum)
            log.info("设置循环日志记录条数: {0}".format(recordCycleNum))

        # 循环日志记录条数
        if outputLogPrintRuler:
            self.browser.find_element_by_xpath("//*[@id='outputLogPrintRuler']/following-sibling::span").clear()
            self.browser.find_element_by_xpath(
                "//*[@id='outputLogPrintRuler']/following-sibling::span").send_keys(outputLogPrintRuler)
    ...
    def set_logic_branch(self, enable, branchType, meetCondition, unMeetCondition, unCertainCondition, dynamicCondition):
        ...
        """
        # 滑动页面
        logic_element = self.browser.find_element_by_xpath("//*[@id='westExap']//h2")
        ...
        if enable:
            ...
            temp = True if enable == "开启" else False
            if not (temp and status):
                self.browser.find_element_by_xpath("//*[@id='_node_logic_tag']").click()
                sleep(1)
        ...
        if branchType:
            self.browser.find_element_by_xpath(
                "//*[@name='branchType']/following-sibling::span[text()='{0}']".format(branchType)).click()
        ...
        if meetCondition:
            # 先清空
            self.browser.find_element_by_xpath(
                "//*[contains(@onclick,'deleteAdd') and contains(@onclick,'fitcnd')]").click()
            # 点击修改
            self.browser.find_element_by_xpath(
                "//*[contains(@onclick,'showAdd') and contains(@onclick,'fitcnd')]").click()
            page_wait()
            iframe_xpath_list = [get_global_var("NodeIframe"), get_global_var("ControlIframe")]
        ...
        if unMeetCondition:
            # 先清空
            self.browser.find_element_by_xpath(
                "//*[contains(@onclick,'deleteAdd') and contains(@onclick,'nofitcnd')]").click()
            # 点击修改
            self.browser.find_element_by_xpath(
                "//*[contains(@onclick,'showAdd') and contains(@onclick,'nofitcnd')]").click()
            page_wait()
            iframe_xpath_list = [get_global_var("NodeIframe"), get_global_var("ControlIframe")]
        ...
        if unCertainCondition:
            # 先清空
            self.browser.find_element_by_xpath(
                "//*[contains(@onclick,'deleteAdd') and contains(@onclick,'unsurecnd')]").click()
            # 点击修改
            self.browser.find_element_by_xpath(
                "//*[contains(@onclick,'showAdd') and contains(@onclick,'unsurecnd')]").click()
            page_wait()
            iframe_xpath_list = [get_global_var("NodeIframe"), get_global_var("ControlIframe")]
        ...
        if dynamicCondition:
            # 先清空
            self.browser.find_element_by_xpath(
                "//*[contains(@onclick,'deleteAdd') and contains(@onclick,'dynamicCnd')]").click()
            # 点击修改
            self.browser.find_element_by_xpath(
                "//*[contains(@onclick,'showAdd') and contains(@onclick,'dynamicCnd')]").click()
            page_wait()
            iframe_xpath_list = [get_global_var("NodeIframe"), get_global_var("ControlIframe")]
    ...
    def save(self):
        self.browser.find_element_by_xpath("//*[@onclick='saveCtrlCfgCondition()']").click()
        alert = BeAlertBox(back_iframe="default")
        msg = alert.get_msg()
        if alert.title_contains("操作成功"):
            log.info("设置成功")
