...
def cmd_temp_fetch(opt, target_var, var_name, obj_type, result_type, cmd_name, value_type):
    ...
    if opt == "添加":
        ...
        browser.switch_to.frame(
            browser.find_element_by_xpath("//iframe[contains(@src,'getdataCmdTemplNodeEdit.html?type=add')]"))
        ...
        sleep(2)
        browser.switch_to.frame(
            browser.find_element_by_xpath("//iframe[contains(@src,'getdataCmdTemplNodeEdit.html?type=edit')]"))
        wait = WebDriverWait(browser, 10)
        wait.until(ec.element_to_be_clickable((By.XPATH, "//*[@name='varName']/preceding-sibling::input")))
        log.info("修改取数配置")
    ...
    # 切换到取数配置iframe
    browser.switch_to.frame(browser.find_element_by_xpath("//iframe[@id='getdata_cmd_templ_node']"))
