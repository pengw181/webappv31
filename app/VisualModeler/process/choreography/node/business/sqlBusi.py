...
def sql_business(node_name, opt_mode, sql_config, advance_set):
    ...
    if opt_mode:
        ...
        browser.find_element_by_xpath(
            "//*[contains(@id,'node_model_id') and text()='{0}']".format(opt_mode)).click()
    ...
    if advance_set:
        if advance_set.get("状态") == "开启":
            ...
            try:
                browser.find_element_by_xpath(
                    "//*[@onclick='show_advanced_mode($(this))']//*[text()='开启高级模式']").click()
                log.info("开启【高级配置】")
            ...
            try:
                ...
                browser.find_element_by_xpath(
                    "//*[@onclick='show_advanced_mode($(this))']//*[text()='关闭高级模式']").click()
                log.info("关闭【高级配置】")
        sleep(1)
    ...
    # 保存业务配置
    browser.find_element_by_xpath("//*[@id='save_sqlContent']//*[text()='保存']").click()
    log.info("保存业务配置")
...
def sql_advance_mode(db, sql):
    ...
    # 选择数据库
    if db:
        browser.find_element_by_xpath("//*[@name='dbId']/preceding-sibling::input").click()
        db_element = browser.find_element_by_xpath("//*[contains(@id,'db_name') and text()='{0}']".format(db))
        browser.execute_script("arguments[0].scrollIntoView(true);", db_element)
        db_element.click()

    # 切换到编写sql的iframe
    browser.switch_to.frame(browser.find_element_by_xpath("//iframe[contains(@src,'sqlNodeAdv.html')]"))

    # 编写sql
    if sql:
        log.info("开始填写sql")
        log.info(sql)
        text_area = browser.find_element_by_xpath("//*[@id='sql_content']")
...
def sql_normal_mode(var, db, storage_mode, table_name, col_map):
    ...
    browser = get_global_var("browser")
    # 点击添加
    browser.find_element_by_xpath("//*[@id='normalCfgAdd']//*[text()='添加']").click()
    page_wait()
    sleep(1)

    # 进入普通模式iframe
    browser.switch_to.frame(browser.find_element_by_xpath("//iframe[contains(@src,'sqlNodeNormalEdit.html?')]"))
    # 等待页面加载
    ...
    if db:
        browser.find_element_by_xpath("//*[@name='dbId']/preceding-sibling::input").click()
        db_element = browser.find_element_by_xpath("//*[contains(@id,'dbId') and text()='{0}']".format(db))
        browser.execute_script("arguments[0].scrollIntoView(true);", db_element)
        db_element.click()
    ...
    # 设置存储模式
    if storage_mode:
        browser.find_element_by_xpath("//*[@name='storageMode']/preceding-sibling::input").click()
        browser.find_element_by_xpath("//*[contains(@id,'storageMode') and text()='{0}']".format(storage_mode)).click()
    ...
    if table_name:
        page_wait()
        table_element = browser.find_element_by_xpath(
            "//*[contains(@id,'tree_easyui_tree_')]//*[text()='{0}']".format(table_name))
    ...
    if col_map:
        ...
        for key, value in col_map.items():
            ...
            # 设置值类型
            browser.find_element_by_xpath(
                "//*[@field='columnChiName']/*[text()='{}']/../following-sibling::td[2]//a".format(key)).click()
            type_element = browser.find_elements_by_xpath(
                "//*[contains(@id,'typeCOL') and text()='{0}']".format(col_type))
            ...
            if col_type == "变量名":
                # 变量
                browser.find_element_by_xpath(
                    "//*[@field='columnChiName']/*[text()='{}']/../following-sibling::td[1]//a".format(key)).click()
                choose_var(var_name=col_value)
            else:
                browser.find_element_by_xpath(
                    "//*[@field='columnChiName']/*[text()='{}']/../following-sibling::td[1]//*[contains(@id,'input')]".format(
                        key)).send_keys(col_value)
            log.info("字段: {0} 映射配置完成".format(key))
            sleep(1)

    # 点击保存
    browser.find_element_by_xpath("//*[@onclick='saves()']//*[text()='保存']").click()
    # 切回业务配置iframe
    browser.switch_to.parent_frame()
