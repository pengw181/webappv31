...
def report_business(node_name, opt_type, obj_var, var_name, var_map, interface_name, remark, sample_data):
    ...
    if opt_type == "添加":
        # 点击添加按钮
        browser.find_element_by_xpath("//*[@onclick='toVarCfg()']//*[text()='添加']").click()
        # 切换到变量配置页面
        page_wait()
        ...
        if obj_var:
            # 双击变量进入修改页面
            obj = browser.find_element_by_xpath("//*[contains(@id,'tableVarCfg')]//*[text()='{0}']".format(obj_var))
        ...
        # 变量名
        if obj_var:
            browser.find_element_by_xpath("//*[contains(@id,'tableVarCfg')]//*[text()='{0}']".format(obj_var)).click()
            # 点击删除按钮
            browser.find_element_by_xpath("//*[@onclick='delVarCfg()']//*[text()='删除']").click()
            alert = BeAlertBox()
            msg = alert.get_msg()
            if alert.title_contains("您确定需要删除{0}吗".format(obj_var), auto_click_ok=False):
                ...
                if not alert.exist_alert:
                    ...
                    msg = alert.get_msg()
                    log.warn("{0} 删除失败，失败提示: {1}".format(obj_var, msg))
            else:
                # 操作异常
                log.warn("{0} 删除失败，失败提示: {1}".format(obj_var, msg))
    ...
    if var_name:
        ...
        browser.find_element_by_xpath("//*[@id='dataH_inputVar2Name']/following-sibling::span//a").click()
    ...
    if var_map:
        ...
        col_set_ele = browser.find_elements_by_xpath(
            "//*[contains(@id,'tableVarCfg')]/*[@field='varIndex']//*[@class='col_item']")
        if len(col_set_ele) > 0:
            for element in col_set_ele:
                element.click()
                browser.find_element_by_xpath("//*[@onclick='deleteRow()']//*[text()='删除']").click()
            log.info("变量索引配置删除{0}条配置".format(len(col_set_ele)))

        # 点击添加按钮
        browser.find_element_by_xpath("//*[@onclick='appendRow()']//*[text()='添加']").click()
        ...
        for col in var_map:
            ...
            if var_index:
                browser.find_element_by_xpath(
                    row_edit_xpath + "/*[@field='varIndex']//input[contains(@id,'textbox')]").clear()
                browser.find_element_by_xpath(
                    row_edit_xpath + "/*[@field='varIndex']//input[contains(@id,'textbox')]").send_keys(var_index)
                log.info("设置变量索引: {0}".format(var_index))

            # 索引说明
            if var_detail:
                browser.find_element_by_xpath(
                    row_edit_xpath + "/*[@field='varDetail']//input[contains(@id,'textbox')]").clear()
                browser.find_element_by_xpath(
                    row_edit_xpath + "/*[@field='varDetail']//input[contains(@id,'textbox')]").send_keys(var_detail)
                log.info("设置索引说明: {0}".format(var_detail))

            # 变量元素类型
            if data_type:
                browser.find_element_by_xpath(row_edit_xpath + "/*[@field='dataType']//a").click()
                sleep(1)
                browser.find_element_by_xpath(
                    "//*[contains(@id,'_easyui_combobox') and text()='{0}']".format(data_type)).click()
                log.info("设置变量元素类型: {0}".format(data_type))

            # 变量元素格式
            if data_format:
                browser.find_element_by_xpath(row_edit_xpath + "/*[@field='dataFormat']//a").click()
                sleep(1)
                browser.find_element_by_xpath(
                    "//*[contains(@id,'_easyui_combobox') and text()='{0}']".format(data_format)).click()
                log.info("设置变量元素格式: {0}".format(data_format))

            # 类型
            if col_type:
                browser.find_element_by_xpath(row_edit_xpath + "/*[@field='colType']//a").click()
                sleep(1)
                browser.find_element_by_xpath(
                    "//*[contains(@id,'_easyui_combobox') and text()='{0}']".format(col_type)).click()
    ...
    # 数据接口名称
    if interface_name:
        browser.find_element_by_xpath("//*[@id='interfaceName']/following-sibling::span/input[1]").clear()
        browser.find_element_by_xpath(
            "//*[@id='interfaceName']/following-sibling::span/input[1]").send_keys(interface_name)
        log.info("设置数据接口名称: {0}".format(interface_name))

    # 备注
    if remark:
        remark_textarea = browser.find_element_by_xpath("//*[@id='remark']/following-sibling::span/textarea")
        set_textarea(textarea=remark_textarea, msg=remark)
        log.info("设置备注")

    # 样例数据
    if sample_data:
        sample_data_area = browser.find_element_by_xpath("//*[@id='sampleData']")
        content = load_sample(sample_file_name=sample_data)
        set_textarea(textarea=sample_data_area, msg=content)
        log.info("设置样例数据")

    # 保存节点变量配置
    browser.find_element_by_xpath("//*[@onclick='saveNodeVarRel();']//*[text()='保存']").click()
    ...
    if alert.title_contains("保存成功"):
        ...
        log.warn("保存节点变量配置失败，失败提示: {0}".format(msg))
    set_global_var("resultMsg", msg, False)
    ...
    # 保存业务配置
    browser.find_element_by_xpath("//*[@id='save_node_info']//*[text()='保存']").click()
    log.info("保存业务配置")
