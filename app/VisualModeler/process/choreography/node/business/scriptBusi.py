...
def script_business(node_name, script_name, ver_no, params, advance_set):
    ...
    # 选择脚本
    if script_name:
        browser.find_element_by_xpath("//*[@id='scriptId']/following-sibling::span//a").click()
        script_element = browser.find_element_by_xpath(
            "//*[contains(@id,'scriptId') and text()='{0}']".format(script_name))
    ...
    # 选择版本
    if ver_no:
        browser.find_element_by_xpath("//*[@id='versionNo']/following-sibling::span//a").click()
        page_wait()
        wait = WebDriverWait(browser, 30)
        wait.until(ec.element_to_be_clickable((
            By.XPATH, "//*[contains(@id,'versionNo') and text()='{0}']".format(ver_no))))

        version_element = browser.find_element_by_xpath(
            "//*[contains(@id,'versionNo') and text()='{0}']".format(ver_no))
    ...
    # 保存业务配置
    browser.find_element_by_xpath("//*[@onclick='saveScriptParamInfo(true)']//*[text()='保存']").click()
    log.info("保存业务配置")
...
def script_param_set(param_key, param_type, param_value):
    ...
    browser = get_global_var("browser")
    # 获取参数所在行号
    row_index = browser.find_element_by_xpath(
        "//*[@field='paramName']/*[text()='{}']/../preceding-sibling::td/div".format(
            param_key)).get_attribute("innerText")
    # 行号从0开始，参数序号从1开始
    row_index = int(row_index)
    # 点击参数类型下拉箭头，展示出参数类型
    browser.find_element_by_xpath("//*[@id='valType_{0}']/following-sibling::span//a".format(row_index)).click()

    # 选择参数类型
    param_type_ele = browser.find_elements_by_xpath(
        "//*[contains(@id,'valType_{0}') and text()='{1}']".format(row_index, param_type))
    ...
    # 设置参赛值
    if param_type == "变量":
        browser.find_element_by_xpath("//*[@id='p_var_{0}']/following-sibling::span//a".format(row_index)).click()
        # 选择变量
        choose_var(var_name=param_value)
    else:
        # 输入固定值内容
        browser.find_element_by_xpath("//*[@id='paraVal_{0}']/following-sibling::span/input[1]".format(row_index)).clear()
        browser.find_element_by_xpath(
            "//*[@id='paraVal_{0}']/following-sibling::span/input[1]".format(row_index)).send_keys(param_value)
