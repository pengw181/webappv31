...
from app.VisualModeler.main.menu.menuXpath import *
...
class DoctorWho:
    ...
    def choose_menu(self, menu_path):
        ...
        try:
            first_menu_element = self.browser.find_element_by_xpath(get_menu_xpath("first", first_menu))
            ...
            if second_menu:
                second_menu_element = self.browser.find_element_by_xpath(get_menu_xpath("second", second_menu))
                ...
                if second_menu not in ("云平台", "告警平台", "数据接入平台", "OA审批平台"):
                    if second_menu not in ["文件目录管理"]:
                        ...
                        third_menu_element = self.browser.find_element_by_xpath(get_menu_xpath("third", third_menu))
