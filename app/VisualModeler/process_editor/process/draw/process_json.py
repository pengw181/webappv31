...
class ProcessNodeJson:
    ...
    def __init__(self, process_name):
        sql = "select json from tn_process_conf_info t where t.PROCESS_NAME='{0}'".format(process_name)
        ...
        self.node_num = 0
        self.node_dict = {}
        self.nodes = jsonpath.jsonpath(self.result_json, "$.nodes")
        log.info("json结果: \n{0}".format(self.result_json))

    def reload(self):
        sql = "select json from tn_process_conf_info t where t.PROCESS_NAME='{0}'".format(self.process_name)
        sql_util = SQLUtil(db=get_global_var("Database"), schema="main")
        result = sql_util.select(sql)
        self.result_json = json.loads(result)
        self.nodes = jsonpath.jsonpath(self.result_json, "$.nodes")
    ...
    def count_node(self):
        ...
        for node in self.nodes:
            self.node_num = len(list(node.keys()))
