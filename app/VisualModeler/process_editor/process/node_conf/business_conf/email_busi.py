...
from common.page.func.choose_dir import choose_file_dir, choose_ftp_dir
...
def email_business(node_name, mode, params_set):
    ...
    if mode:
        ...
        browser.find_element_by_xpath("//*[contains(@id,'node_model_id') and text()='{0}']".format(mode)).click()
    ...
    if operate_mode == "接收":
        email_receive(receiver_email=params_set.get("接收邮箱"), begin_time=params_set.get("发件开始时间"),
                      end_time=params_set.get("发件结束时间"), receiver=params_set.get("收件人"), sender=params_set.get("发件人"),
                      email_title=params_set.get("标题"), email_content=params_set.get("正文"), attach=params_set.get("附件"),
    ...
    # 保存业务配置
    browser.find_element_by_xpath("//*[@id='emailSaveBtn']//*[text()='保存']").click()
    log.info("保存业务配置")
...
def email_receive(receiver_email, begin_time, end_time, receiver, sender, email_title, email_content, attach,
    ...
    page_wait()
    # 切换到接收模式配置iframe
    browser.switch_to.frame(browser.find_element_by_xpath("//iframe[contains(@src,'emailReceive.html')]"))
    wait = WebDriverWait(browser, 30)
    wait.until(ec.element_to_be_clickable((By.XPATH, "//*[@name='account']/preceding-sibling::input")))

    # 接收邮箱
    if receiver_email:
        browser.find_element_by_xpath("//*[@name='account']/preceding-sibling::input").click()
        browser.find_element_by_xpath("//*[contains(@id,'account') and text()='{0}']".format(receiver_email)).click()
    ...
    if begin_time:
        ...
        # 聚焦元素
        use_var = browser.find_element_by_xpath("//*[@id='userVar_1']")
        ...
        if begin_time.get("变量引用") == "是":
            ...
            form_selector = "//*[@id='form_id']"
            set_text_enable_var(input_xpath=input_xpath, form_selector=form_selector, msg=begin_time.get("值"))
        else:
            if status:
                use_var.click()
            ...
            # 值
            set_date(date_s=begin_time.get("值"), date_format="%Y-%m-%d %H:%M")
        sleep(1)
    ...
    if end_time:
        ...
        # 聚焦元素
        use_var = browser.find_element_by_xpath("//*[@id='userVar_2']")
        action.move_to_element(use_var).perform()
        if end_time.get("变量引用") == "是":
            ...
            form_selector = "//*[@id='form_id']"
            set_text_enable_var(input_xpath=input_xpath, form_selector=form_selector, msg=end_time.get("值"))
        else:
            if status:
                use_var.click()
            ...
            # 值
            set_date(date_s=end_time.get("值"), date_format="%Y-%m-%d %H:%M")
        sleep(1)
    ...
    if receiver:
        ...
        # 聚焦元素
        use_regular = browser.find_element_by_xpath("//*[@id='isRegex_1']")
        ...
        if receiver.get("正则匹配") == "是":
            ...
            # 值
            browser.find_element_by_xpath("//*[@name='keyExpr1']/preceding-sibling::span/a").click()
            # 切换到正则配置iframe
            browser.switch_to.frame(
                browser.find_element_by_xpath("//iframe[contains(@src,'operateWashRegexBox.html')]"))
            regular = receiver.get("值")
            if regular.get("高级模式") == "是":
                advance_mode = True
            else:
                advance_mode = False
            regular_cube(set_type=regular.get("设置方式"), regular_name=regular.get("正则模版名称"),
                         advance_mode=advance_mode, regular=regular.get("标签配置"), expression=regular.get("表达式"))
            if regular.get("设置方式") == "添加":
                alert = BeAlertBox(back_iframe="default")
                msg = alert.get_msg()
                if alert.title_contains("成功"):
                    ...
                    # 切换到业务配置iframe
                    browser.switch_to.frame(browser.find_element_by_xpath("//iframe[@id='busi_node']"))
                    # 切换到接收模式配置iframe
                    browser.switch_to.frame(browser.find_element_by_xpath("//*[contains(@src,'emailReceive.html')]"))
                else:
            ...
            browser.find_element_by_xpath(
                "//*[text()='正则魔方']/following-sibling::div/a[contains(@class,'close')]").click()
            sleep(1)
        else:
            if status:
                use_regular.click()
            ...
            form_selector = "//*[@id='form_id']"
            set_text_enable_var(input_xpath=input_xpath, form_selector=form_selector, msg=receiver.get("值"))
            sleep(1)
    ...
    if sender:
        ...
        # 聚焦元素
        use_regular = browser.find_element_by_xpath("//*[@id='isRegex_2']")
        action.move_to_element(use_regular).perform()
        if sender.get("正则匹配") == "是":
            ...
            # 值
            browser.find_element_by_xpath("//*[@name='keyExpr2']/preceding-sibling::span/a").click()
            # 切换到正则配置iframe
            browser.switch_to.frame(browser.find_element_by_xpath("//iframe[contains(@src,'operateWashRegexBox.html')]"))
            ...
            if regular.get("设置方式") == "添加":
                ...
                if alert.title_contains("成功"):
                    ...
                    # 切换到接收模式配置iframe
                    browser.switch_to.frame(
                        browser.find_element_by_xpath("//*[contains(@src,'emailReceive.html')]"))
                else:
                    log.warn("保存正则模版失败，失败提示: {0}".format(msg))
                set_global_var("resultMsg", msg, False)
            else:
            ...
            form_selector = "//*[@id='form_id']"
            set_text_enable_var(input_xpath=input_xpath, form_selector=form_selector, msg=sender.get("值"))
            sleep(1)
    ...
    if email_title:
        ...
        # 聚焦元素
        use_regular = browser.find_element_by_xpath("//*[@id='isRegex_3']")
        action.move_to_element(use_regular).perform()
        if email_title.get("正则匹配") == "是":
            ...
            log.info("勾选【标题 正则匹配】")
            # 值
            browser.find_element_by_xpath("//*[@name='keyExpr3']/preceding-sibling::span/a").click()
            # 切换到正则配置iframe
            browser.switch_to.frame(
                browser.find_element_by_xpath("//iframe[contains(@src,'operateWashRegexBox.html')]"))
            ...
            form_selector = "//*[@id='form_id']"
            set_text_enable_var(input_xpath=input_xpath, form_selector=form_selector, msg=email_title.get("值"))
            sleep(1)
    ...
    if email_content:
        ...
        # 聚焦元素
        use_regular = browser.find_element_by_xpath("//*[@id='isRegex_4']")
        action.move_to_element(use_regular).perform()
        if email_content.get("正则匹配") == "是":
            ...
            log.info("勾选【正文 正则匹配】")
            # 值
            browser.find_element_by_xpath("//*[@name='keyExpr4']/preceding-sibling::span/a").click()
            # 切换到正则配置iframe
            browser.switch_to.frame(
                browser.find_element_by_xpath("//iframe[contains(@src,'operateWashRegexBox.html')]"))
            ...
            form_selector = "//*[@id='form_id']"
            set_text_enable_var(input_xpath=input_xpath, form_selector=form_selector, msg=email_content.get("值"))
            sleep(1)
    ...
    if attach:
        ...
        # 聚焦元素
        use_regular = browser.find_element_by_xpath("//*[@id='isRegex_5']")
        action.move_to_element(use_regular).perform()
        if attach.get("正则匹配") == "是":
            ...
            log.info("勾选【附件 正则匹配】")
            # 值
            browser.find_element_by_xpath("//*[@name='keyExpr5']/preceding-sibling::span/a").click()
            # 切换到正则配置iframe
            browser.switch_to.frame(
                browser.find_element_by_xpath("//iframe[contains(@src,'operateWashRegexBox.html')]"))
            ...
            form_selector = "//*[@id='form_id']"
            set_text_enable_var(input_xpath=input_xpath, form_selector=form_selector, msg=attach.get("值"))
            sleep(1)
    ...
    if attach_type:
        ...
        for i in attach_type:
            browser.find_element_by_xpath("//*[contains(@id,'attachtype') and text()='{0}']".format(i)).click()
    ...
    if storage:
        ...
        # 聚焦元素
        save_attach = browser.find_element_by_xpath("//*[@id='isSaveAttach']")
        browser.execute_script("arguments[0].scrollIntoView(true);", save_attach)
        if storage.get("flag") == "是":
            if not status:
                save_attach.click()
            log.info("勾选【存储附件】")
            # 目录
            browser.find_element_by_xpath("//*[@name='catagory_id']/preceding-sibling::input").click()
            choose_file_dir(dir_name=storage.get("目录"))
        else:
            if status:
                save_attach.click()
                log.info("取消勾选【存储附件】")
            else:
                log.info("【存储附件】标识为否，不开启")
...
def email_send(sender, receiver, email_title, email_content, cc, attach):
    ...
    # 切换到信息推送iframe
    page_wait()
    browser.switch_to.frame(browser.find_element_by_xpath("//*[contains(@src,'emailSend.html')]"))
    wait = WebDriverWait(browser, 30)
    ...
    # 发件人
    if sender:
        browser.find_element_by_xpath("//*[@id='sender']/following-sibling::span//a").click()
        browser.find_element_by_xpath("//*[contains(@id,'sender') and text()='{0}']".format(sender)).click()
    ...
    if receiver:
        ...
        if receiver.__contains__("类型"):
            ...
            if receiver_type == "自定义":
                browser.find_element_by_xpath("//*[@id='getrecipientsBtn_1']").click()
            else:
                browser.find_element_by_xpath("//*[@id='getrecipientsBtn_2']").click()
            log.info("设置收件人类型: {0}".format(receiver_type))

        # 方式。因无法通过js获取方式下拉框的值，所以方式设置为必须参数
        choose_type = receiver.get("方式")
        browser.find_element_by_xpath("//*[@id='selUser1']/following-sibling::span//input[1]").click()
        browser.find_element_by_xpath("//*[contains(@id,'selUser1') and text()='{0}']".format(choose_type)).click()
        ...
        if choose_type == "请选择":
            browser.find_element_by_xpath("//*[@id='recipients_sel']/following-sibling::span//a").click()
            for i in receiver_obj:
                browser.find_element_by_xpath("//*[contains(@id,'recipients_sel') and text()='{0}']".format(i)).click()
            # 再点击一次，隐藏下拉框
            browser.find_element_by_xpath("//*[@id='recipients_sel']/following-sibling::span//a").click()
            log.info("设置收件人: {0}".format(",".join(receiver_obj)))
        else:
            # 自定义
            browser.find_element_by_xpath("//*[@id='dataH_recipientsName']/following-sibling::span//a").click()
    ...
    # 抄送人
    if cc:
        browser.find_element_by_xpath("//*[@id='selUser2']/following-sibling::span//input[1]").click()
        # 方式
        choose_type = cc.get("方式")
        browser.find_element_by_xpath("//*[contains(@id,'selUser2') and text()='{0}']".format(choose_type)).click()
        ...
        cc_obj = cc.get("值")
        if choose_type == "请选择":
            browser.find_element_by_xpath("//*[@id='copySenders']/following-sibling::span//a").click()
            for i in cc_obj:
                browser.find_element_by_xpath("//*[contains(@id,'copySenders') and text()='{0}']".format(i)).click()
            # 再点击一次，隐藏下拉框
            browser.find_element_by_xpath("//*[@id='copySenders']/following-sibling::span//a").click()
            log.info("设置抄送人: {0}".format(",".join(cc_obj)))
        else:
            browser.find_element_by_xpath("//*[@id='dataH_copySendersName']/following-sibling::span//a").click()
    ...
    if email_title:
        ...
        form_selector = "//*[@id='email_send_form']"
        set_text_enable_var(input_xpath=input_xpath, form_selector=form_selector, msg=email_title)
        log.info("设置标题: {0}".format(email_title))
        sleep(1)

    # 正文
    if email_content:
        text_area = browser.find_element_by_xpath("//*[@id='email_content']")
        action = ActionChains(browser)
...
def attach_opt(opt, attach):
    ...
    """
    browser = get_global_var("browser")
    attach_obj = browser.find_element_by_xpath("//*[@onclick='toAddAttachPage()']//*[text()='添加']")
    browser.execute_script("arguments[0].scrollIntoView(true);", attach_obj)
    # 操作类型
    if opt == "添加":
        browser.find_element_by_xpath("//*[@onclick='toAddAttachPage()']//*[text()='添加']").click()
        sleep(1)
        # 切换到附件配置iframe，这里有两层
        wait = WebDriverWait(browser, 30)
        ...
        if attach.__contains__("附件来源"):
            ...
            if attach_source == "动态生成":
                browser.find_element_by_xpath("//*[@id='dybuild']").click()
            elif attach_source == "本地上传":
                browser.find_element_by_xpath("//*[@id='localUp']").click()
            elif attach_source == "远程加载":
                browser.find_element_by_xpath("//*[@id='remoteUp']").click()
            else:
        ...
        if attach_source == "动态生成":
            ...
            browser.find_element_by_xpath("//*[contains(@for,'filebox_file_id')]").click()
            sleep(1)
            upload(file_name=attach.get("文件名"))
        else:
            use_var = attach.get("变量引用")
            if use_var == "是":
                use_var = True
            else:
                use_var = False
            attach_remote_up(storage_type=attach.get("存储类型"), ftp=attach.get("远程服务器"),
                             use_var=use_var, dir_name=attach.get("目录"), filter_type=attach.get("过滤类型"),
                             file_name=attach.get("文件名"), file_type=attach.get("附件类型"))
    elif opt == "修改":
        # TODO
        log.info("附件修改成功")
    else:
    ...
    # 点击保存
    browser.find_element_by_xpath("//*[@onclick='saveAttachInfoCom()']//*[text()='保存']").click()
    alert = BeAlertBox(back_iframe="default")
    msg = alert.get_msg()
    if alert.title_contains("成功"):
        log.info("保存附件成功")
    else:
        log.warn("保存附件失败，失败提示: {0}".format(msg))
    set_global_var("ResultMsg", msg, False)
...
def attach_build(attach_tile, attach_content, attach_type):
    ...
    if attach_tile:
        ...
        form_selector = "//*[@id='attachFile_form']"
        set_text_enable_var(input_xpath=input_xpath, form_selector=form_selector, msg=attach_tile)
    ...
    if attach_content:
        ...
        form_selector = "//*[@id='attachFile_form']"
        set_text_enable_var(input_xpath=input_xpath, form_selector=form_selector, msg=attach_content)
    ...
    if attach_type:
        browser.find_element_by_xpath("//*[@name='attachType']/preceding-sibling::input").click()
        browser.find_element_by_xpath("//*[contains(@id,'attachType') and text()='{0}']".format(attach_type)).click()
...
def attach_remote_up(storage_type, ftp, use_var, dir_name, filter_type, file_name, file_type):
    ...
    if storage_type:
        if storage_type == "本地":
            browser.find_element_by_xpath("//*[@id='remote_local']").click()
        else:
            browser.find_element_by_xpath("//*[@id='remote_server']").click()
    ...
    if storage_type == "本地":
        ...
        # 聚焦元素
        enable_var = browser.find_element_by_xpath("//*[@id='local_isKeyword']")
        ...
        if use_var:
            ...
            browser.find_element_by_xpath(
                "//*[@id='local_choose_span']//*[@name='srcPath']/preceding-sibling::input").click()
            choose_file_dir(dir_name=dir_name)
            log.info("设置目录: {0}".format(dir_name))
            sleep(1)
    else:
        # 选择ftp
        browser.find_element_by_xpath("//*[@id='remote_srcServerId']/following-sibling::span//input[1]").click()
        browser.find_element_by_xpath("//*[contains(@id,'remote_srcServerId') and text()='{0}']".format(ftp)).click()
        log.info("选择ftp: {0}".format(ftp))
        sleep(1)

        # 选择ftp目录
        browser.find_element_by_xpath("//*[@name='srcPath']/preceding-sibling::input").click()
    ...
    # 过滤类型
    if filter_type:
        browser.find_element_by_xpath("//*[@id='fileFilterType1']/following-sibling::span//input[1]").click()
        browser.find_element_by_xpath("//*[contains(@id,'fileFilterType') and text()='{0}']".format(filter_type)).click()
        log.info("选择过滤类型: {0}".format(filter_type))
        sleep(1)

    # 获取过滤类型当前选定类型
    filter_ele = browser.find_element_by_xpath("//*[@id='fileFilterType1']/following-sibling::span//input[2]")
    ...
    if filter_type == "关键字":
        ...
        browser.find_element_by_xpath("//*[@id='keyExpr2']/following-sibling::span//a[1]").click()
        sleep(1)
        browser.switch_to.frame(
            browser.find_element_by_xpath(
                "//iframe[contains(@src,'/VisualModeler/frame/regexcube/regexpTmplPopUpWin.html')]"))
        confirm_selector = "//*[@id='regexpPopUp']"
        regular_cube(confirm_selector=confirm_selector, set_type=file_name.get("设置方式"),
        ...
        if file_name.get("设置方式") == "添加":
            ...
            if alert.title_contains("成功"):
                ...
                # 切换到接收模式配置iframe
                browser.switch_to.frame(
                    browser.find_element_by_xpath("//*[contains(@src,'emailSend.html')]"))
                # 切换到添加附件iframe
                browser.switch_to.frame(browser.find_element_by_xpath("//iframe[contains(@src,'addEmailAttach.html')]"))
                # 切换到附件配置iframe
                browser.switch_to.frame(
                    browser.find_element_by_xpath("//iframe[contains(@src,'attachFileUpload.html')]"))
                # 切换到配置正则iframe
                browser.switch_to.frame(
                    browser.find_element_by_xpath(
                        "//iframe[contains(@src,'/VisualModeler/frame/regexcube/regexpTmplPopUpWin.html')]"))
    ...
    if file_type:
        ...
        browser.find_element_by_xpath("//*[contains(@id,'fileType1') and text()='{0}']".format(file_type)).click()
