...
def alarm_business(node_name, alarm_data_type, alarm_plan, alarm_rule, invoke_syn, advance_set):
    ...
    # 选择告警类型
    if alarm_data_type:
        browser.find_element_by_xpath("//*[@id='alarmDataType']/following-sibling::span//a").click()
        alarm_type_element = browser.find_element_by_xpath(
            "//*[contains(@id,'alarmDataType') and text()='{0}']".format(alarm_data_type))
    ...
    # 选择告警计划
    if alarm_plan:
        browser.find_element_by_xpath("//*[@id='alarmPlanId']/following-sibling::span//a").click()
        alarm_plan_element = browser.find_element_by_xpath(
            "//*[contains(@id,'alarmPlanId') and text()='{0}']".format(alarm_plan))
    ...
    # 选择告警规则
    if alarm_rule:
        browser.find_element_by_xpath("//*[@id='alarmRuleId']/following-sibling::span//a").click()
        alarm_rule_element = browser.find_element_by_xpath(
            "//*[contains(@id,'alarmRuleId') and text()='{0}']".format(alarm_rule))
    ...
    invokeSyn_element = browser.find_element_by_xpath("//*[@for='invokeSyn']")
    ...
    if advance_set:
        if advance_set.get("flag") == "是":
            ...
            browser.find_element_by_xpath("//*[@name='invokeTimeout']/preceding-sibling::input").clear()
            browser.find_element_by_xpath("//*[@name='invokeTimeout']/preceding-sibling::input").send_keys(timeout)
            browser.find_element_by_xpath("//*[@name='retryTimes']/preceding-sibling::input").clear()
            browser.find_element_by_xpath("//*[@name='retryTimes']/preceding-sibling::input").send_keys(retry_times)
            log.info("设置高级模式")
            sleep(1)
        else:
            try:
    ...
    # 保存业务配置
    browser.find_element_by_xpath("//*[@onclick='saveAlarmInfo()']//*[text()='保存']").click()
    log.info("保存业务配置")
