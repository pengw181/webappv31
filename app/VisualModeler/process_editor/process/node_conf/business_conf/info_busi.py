...
from common.page.func.choose_dir import choose_file_dir
...
def info_business(node_name, mode, info_desc, show, info, download_set, var_name):
    ...
    if operate_mode == "结果呈现/下载":
        # 切换到信息配置iframe
        browser.switch_to.frame(browser.find_element_by_xpath("//*[contains(@src,'resultNodeInfoCfg.html')]"))

        # 信息描述
        if info_desc:
            browser.find_element_by_xpath("//*[@name='resultDesc']/preceding-sibling::input").clear()
            browser.find_element_by_xpath("//*[@name='resultDesc']/preceding-sibling::input").send_keys(info_desc)
        ...
        # 聚焦元素
        is_show = browser.find_element_by_xpath("//*[@for='isshow']")
        ...
        # 信息明细
        if info:
            text_area = browser.find_element_by_xpath("//*[@id='resultDetail']")
        ...
        if download_set:
            ...
            # 聚焦元素
            enable_download = browser.find_element_by_xpath("//*[@for='isdownload']")
            browser.execute_script("arguments[0].scrollIntoView(true);", enable_download)
            if download_set.get("flag") == "是":
                if not status:
                    ...
                    if download_dir:
                        ...
                        if num > 1:
                            browser.find_element_by_xpath(
                                "//*[@id='file_name{0}']/../following-sibling::div[1]/a[@onclick='addFileDownPath()']".format(
                                    num - 1)).click()
                        flag = False
                ...
                for f in file_set:
                    ...
                    if f != file_set[-1]:
                        browser.find_element_by_xpath(
                            "//*[@id='downloadDiv']/div[{0}]//*[@onclick='addFileDownPath()']".format(num)).click()
                        num += 1
            else:
                if status:
                    enable_download.click()
        ...
        # browser.switch_to.frame(browser.find_element_by_xpath("//*[contains(@src,'noticeNodeInfoCfg.html')]"))
        wait = WebDriverWait(browser, 30)
        wait.until(ec.element_to_be_clickable((By.XPATH, "//*[@id='dataH_noticeVarName']/following-sibling::span//a")))

        # 变量选择
        if var_name:
            browser.find_element_by_xpath("//*[@id='dataH_noticeVarName']/following-sibling::span//a").click()
    ...
    # 保存业务配置
    browser.find_element_by_xpath("//*[@id='saveInfoHandleBtn']//*[text()='保存']").click()
    log.info("保存业务配置")
...
def set_download_file(file_path, file_name, row_num):
    ...
    # 目录
    browser.find_element_by_xpath(
        "//*[@name='catagoryId{0}']/preceding-sibling::input".format(num)).click()
    choose_file_dir(dir_name=file_path)

    # 文件名
    browser.find_element_by_xpath(
        "//*[@name='file_name{0}']/preceding-sibling::input".format(num)).clear()
    input_xpath = "//*[@name='file_name{0}']/preceding-sibling::input".format(num)
    form_selector = "//*[@id='info_handle_form']"
