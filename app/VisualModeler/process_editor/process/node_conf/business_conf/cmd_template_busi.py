...
def cmd_template_business(node_name, cmd_temp_name, use_temp_name, advance_set):
    ...
    :param use_temp_name: 应用指令模版名称，bool
    ...
    if cmd_temp_name:
        log.info("开始选择指令模版")
        browser.find_element_by_xpath("//*[@id='cmdTemplateName']/following-sibling::span[1]//a").click()
        browser.switch_to.frame(
            browser.find_element_by_xpath("//iframe[contains(@src, '../../cmd/queryCmdTmplSubPage.html?')]"))
        # 等待页面加载
        page_wait()
        ...
        sleep(1)
        # 输入指令模版名称
        browser.find_element_by_xpath("//*[text()='模版名称']/following-sibling::span[1]/input[1]").send_keys(cmd_temp_name)
        page_wait()
        # 点击查询
        browser.find_element_by_xpath("//*[@id='cmdTmpl-query']//*[text()='查询']").click()
        page_wait()
        # 勾选指令模版
        browser.find_element_by_xpath("//*[@field='templName']//*[text()='{0}']".format(cmd_temp_name)).click()
        # 点击确定
        browser.find_element_by_xpath("//*[@id='cmdTmplSelect']//*[text()='确定']").click()
        log.info("指令模版选择完成")
        sleep(1)
        # 返回上层iframe
    ...
    # 聚焦元素
    enable_click = browser.find_element_by_xpath("//*[@id='is_use_cmd_template_name']")
