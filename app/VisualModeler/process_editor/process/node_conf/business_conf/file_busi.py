...
from common.page.func.choose_dir import choose_ftp_dir, choose_file_dir
...
def file_business(node_name, operate_mode, storage_set, source_set, dest_set, files_set):
    ...
    # 设置操作模式
    if operate_mode:
        browser.find_element_by_xpath("//*[@name='node_model_id']/preceding-sibling::input[1]").click()
        browser.find_element_by_xpath(
            "//*[contains(@id,'node_model_id') and text()='{0}']".format(operate_mode)).click()
    ...
    # 切换到文件配iframe
    browser.switch_to.frame(browser.find_element_by_xpath("//iframe[@id='file_model_content']"))
    # 等待页面加载
    ...
    # 保存业务配置
    browser.find_element_by_xpath("//*[@id='save_irContent']/span/span[1]").click()
    log.info("保存业务配置")
...
def file_storage_mode(storage_set, files_set):
    ...
    # 存储类型
    if storage_set:
        if storage_set.get("变量引用") == "是":
            flag = True
        else:
            flag = False
        file_storage_mode_dir_set(storage_type=storage_set.get("存储类型"), use_var=flag,
    ...
    while flag:
        try:
            var = browser.find_element_by_xpath(
                "//*[@id='dataH_inputVar{0}Name']/following-sibling::span/*[@class='textbox-value']".format(num))
            ...
            if var_name:
                ...
                if num > 1:
                    browser.find_element_by_xpath(
                        "//*[@name='timeFormat{0}']/../../following-sibling::div/a[@onclick='addFilePath(this)']".format(
                            num - 1)).click()
                flag = False
...
def file_storage_mode_dir_set(storage_type, use_var, ftp, dir_name):
    ...
    if storage_type == "本地":
        log.info("配置本地文件")
        browser.find_element_by_xpath("//*[@onclick=\"getStorageType(this,'0')\"]").click()
        ...
        # 聚焦元素
        enable_click = browser.find_element_by_xpath("//*[@id='local_isKeyword']")
        ...
        if use_var:
            ...
            # 目录
            if dir_name:
                browser.find_element_by_xpath("//*[@name='local_keyword']/preceding-sibling::input").clear()
                input_xpath = "//*[@name='local_keyword']/preceding-sibling::input"
                form_selector = "//*[@id='file_storage_form']"
        ...
        browser.find_element_by_xpath("//*[@onclick=\"getStorageType(this,'1')\"]").click()
        # 选择ftp
        browser.find_element_by_xpath("//*[@name='srcServerId']/preceding-sibling::input[1]").click()
        server_element = browser.find_element_by_xpath(
            "//*[contains(@id,'remote_srcServerId') and text()='{0}']".format(ftp))
        browser.execute_script("arguments[0].scrollIntoView(true);", server_element)
        server_element.click()
        log.info("选择ftp: {0}".format(ftp))
        # 点击目录
        browser.find_element_by_xpath("//*[@name='srcPath_2']/preceding-sibling::input[1]").click()
...
def file_storage_mode_file_set(input_var, file_name, file_type, encoding, seperator, sheet_name, add_prefix,
    ...
    num = row_num
    # 聚焦元素
    row_element = browser.find_element_by_xpath(
            "//*[@id='filePathTab']/div[{0}]//*[contains(@data-options,'chooseFileVar(e)')]/following-sibling::span/span/a".format(
                num))
    ...
    if file_type:
        browser.find_element_by_xpath("//*[@name='fileType{0}']/preceding-sibling::input".format(num)).click()
        browser.find_element_by_xpath(
            "//*[contains(@id,'fileType{0}') and text()='{1}']".format(num, file_type)).click()
        log.info("选择文件类型: {0}".format(file_type))
        sleep(1)

    # 设置编码格式
    if encoding:
        browser.find_element_by_xpath("//*[@name='fileCode{0}']/preceding-sibling::input".format(num)).click()
        browser.find_element_by_xpath(
            "//*[contains(@id,'fileCode{0}') and text()='{1}']".format(num, encoding)).click()
    ...
    if seperator:
        browser.find_element_by_xpath(
            "//*[@name='seperator{0}']/preceding-sibling::input".format(num)).send_keys(seperator)
    ...
    if sheet_name:
        browser.find_element_by_xpath(
            "//*[@name='sheetName{0}']/preceding-sibling::input".format(num)).send_keys(sheet_name)
    ...
    # 聚焦元素
    add_prefix_click = browser.find_element_by_xpath("//*[@id='prefixTime{0}']".format(num))
    ...
    # 聚焦元素
    add_suffix_click = browser.find_element_by_xpath("//*[@id='suffixTime{0}']".format(num))
    ...
    # 设置时间格式
    if time_format:
        browser.find_element_by_xpath("//*[@name='timeFormat{0}']/preceding-sibling::input".format(num)).click()
        browser.find_element_by_xpath(
            "//*[contains(@id,'timeFormat{0}') and text()='{1}']".format(num, time_format)).click()
...
def file_cp_mode(src_set, dest_set, files_set):
    ...
    while flag:
        ...
        if choose_type == "0":
            ...
            if num > 1:
                browser.find_element_by_xpath(
                    "//*[@id='oprt_flag{0}']/../following-sibling::div/a[@onclick='addCopyFilePath(this)']".format(
                        num - 1)).click()
            break
        ...
        # 根据类型获取文件名输入框的内容是否为空
        if choose_type == "关键字":
            file_name_element = browser.find_element_by_xpath("//*[@name='src_filepath{0}']".format(num))
        else:
            file_name_element = browser.find_element_by_xpath("//*[@name='keyExprf{0}']".format(num))
...
def file_cp_mode_src_set(storage_type, use_var, ftp, dir_name):
    ...
    if storage_type == "本地":
        log.info("配置本地文件")
        browser.find_element_by_xpath("//*[@name='storageType_1' and @onclick=\"getStorageType(this,'0')\"]").click()
        ...
        # 聚焦元素
        enable_click = browser.find_element_by_xpath("//*[@id='local_isKeyword_1']")
        browser.execute_script("arguments[0].scrollIntoView(true);", enable_click)
        ...
        if use_var:
            ...
            if dir_name:
                browser.find_element_by_xpath(
                    "//*[@id='local_keyword_span_1']//*[@name='local_keyword']/preceding-sibling::input").clear()
                browser.find_element_by_xpath(
                    "//*[@id='local_keyword_span_1']//*[@name='local_keyword']/preceding-sibling::input").send_keys(
                    dir_name)
            ...
            # 源目录
            browser.find_element_by_xpath(
                "//*[@id='local_srcPath_1']/following-sibling::span//*[@name='srcPath']/preceding-sibling::input").click()
            choose_file_dir(dir_name=dir_name)
        sleep(1)

    elif storage_type == "远程":
        log.info("配置远程ftp文件")
        browser.find_element_by_xpath("//*[@name='storageType_1' and @onclick=\"getStorageType(this,'1')\"]").click()

        # 选择ftp
        browser.find_element_by_xpath(
            "//*[@id='remote_srcServerId_1']/following-sibling::span//*[@name='srcServerId']/preceding-sibling::input[1]").click()
        server_element = browser.find_element_by_xpath(
            "//*[contains(@id,'remote_srcServerId_1') and text()='{0}']".format(ftp))
        browser.execute_script("arguments[0].scrollIntoView(true);", server_element)
        server_element.click()
        log.info("选择ftp: {0}".format(ftp))
        ...
        # 源ftp目录
        browser.find_element_by_xpath(
            "//*[@id='remote_choose_span_1']//*[@name='srcPath_2']/preceding-sibling::input[1]").click()
        choose_ftp_dir(path=dir_name)
...
def file_cp_mode_dest_set(storage_type, use_var, ftp, dir_name):
    ...
    if storage_type == "本地":
        log.info("配置本地文件")
        browser.find_element_by_xpath("//*[@name='storageType_2' and @onclick=\"getStorageType(this,'0')\"]").click()
        ...
        # 聚焦元素
        enable_click = browser.find_element_by_xpath("//*[@id='local_isKeyword_2']")
        browser.execute_script("arguments[0].scrollIntoView(true);", enable_click)
        ...
        if use_var:
            ...
            if dir_name:
                browser.find_element_by_xpath(
                    "//*[@id='local_keyword_span_2']//*[@name='local_keyword']/preceding-sibling::input").clear()
                browser.find_element_by_xpath(
                    "//*[@id='local_keyword_span_2']//*[@name='local_keyword']/preceding-sibling::input").send_keys(
                    dir_name)
                log.info("设置目录: {0}".format(dir_name))
        else:
            if status:
            ...
            # 源目录
            browser.find_element_by_xpath(
                "//*[@id='local_srcPath_2']/following-sibling::span//*[@name='srcPath']/preceding-sibling::input").click()
            choose_file_dir(dir_name=dir_name)
        sleep(1)

    elif storage_type == "远程":
        log.info("配置远程ftp文件")
        browser.find_element_by_xpath("//*[@name='storageType_2' and @onclick=\"getStorageType(this,'1')\"]").click()

        # 选择ftp
        browser.find_element_by_xpath(
            "//*[@id='remote_srcServerId_2']/following-sibling::span//*[@name='srcServerId']/preceding-sibling::input[1]").click()
        server_element = browser.find_element_by_xpath(
            "//*[contains(@id,'remote_srcServerId_2') and text()='{0}']".format(ftp))
        browser.execute_script("arguments[0].scrollIntoView(true);", server_element)
        server_element.click()
        log.info("选择ftp: {0}".format(ftp))
        ...
        # 目标ftp目录
        browser.find_element_by_xpath(
            "//*[@id='remote_choose_span_2']//*[@name='srcPath_2']/preceding-sibling::input[1]").click()
        choose_ftp_dir(path=dir_name)
        log.info("远程ftp目录选择完成")
...
def file_cp_mode_file_set(choose_type, file_name, dest_file_name, mode, row_num):
    ...
    num = row_num
    # 聚焦元素
    row_element = browser.find_element_by_xpath("//*[@name='file_choose_type{0}']/preceding-sibling::input".format(num))
    browser.execute_script("arguments[0].scrollIntoView(true);", row_element)

    # 点击选择类型
    if choose_type:
        browser.find_element_by_xpath(
            "//*[@name='file_choose_type{0}']/preceding-sibling::input".format(num)).click()
        browser.find_element_by_xpath(
            "//*[contains(@id,'file_choose_type{0}') and text()='{1}']".format(num, choose_type)).click()
        sleep(1)
    ...
    if choose_type == "关键字":
        ...
        form_selector = "//*[@id='file_cpmv_form']"
        set_text_enable_var(input_xpath=input_xpath, form_selector=form_selector, msg=file_name)
        sleep(1)
    else:
        browser.find_element_by_xpath(
            "//*[@name='keyExprf{0}']/preceding-sibling::span/a".format(num)).click()
        sleep(1)
        # 切换到正则配置iframe页面
    ...
    if dest_file_name:
        browser.find_element_by_xpath(
            "//*[@id='dest_filepath{0}']/following-sibling::span/input[1]".format(num)).clear()
        browser.find_element_by_xpath(
            "//*[@id='dest_filepath{0}']/following-sibling::span/input[1]".format(num)).send_keys(dest_file_name)
        sleep(1)

    # 设置模式，拷贝或移动
    if mode:
        browser.find_element_by_xpath(
            "//*[@id='oprt_flag{0}']/following-sibling::span/input[1]".format(num)).click()
        browser.find_element_by_xpath(
            "//*[contains(@id,'oprt_flag{0}_') and text()='{1}']".format(num, mode)).click()
        sleep(1)
...
def file_load_mode(storage_set, files_set):
    ...
    if storage_set:
        ...
        file_load_mode_dir_set(storage_type=storage_set.get("存储类型"), dir_name=storage_set.get("目录"),
                               use_var=use_var, ftp=storage_set.get("远程服务器"))
    ...
    if files_set:
        ...
        while flag:
            ...
            if choose_type == "0":
                ...
                if num > 1:
                    browser.find_element_by_xpath(
                        "//*[@id='isFilter{0}']/../../following-sibling::div/a[@onclick='addLoadFilePath(this)']".format(
                            num - 1)).click()
                break
            log.info("类型转义: {0}".format(choose_type))
            ...
            # 根据类型获取文件名输入框的内容是否为空
            if choose_type == "关键字":
                file_name_element = browser.find_element_by_xpath("//*[@name='filepath{0}']".format(num))
            else:
                file_name_element = browser.find_element_by_xpath("//*[@name='keyExpr{0}']".format(num))
            var_name = file_name_element.get_attribute("value")
            log.info(var_name)
            if var_name:
                # 如果value不为空，表示本条已配置
...
def file_load_mode_dir_set(storage_type, dir_name, use_var, ftp):
    ...
    if storage_type == "本地":
        ...
        if use_var:
            # 勾选变量引用
            browser.find_element_by_xpath("//*[@id='local_isKeyword']").click()
            # 手动输入目录
            browser.find_element_by_xpath("//*[@name='local_keyword']/preceding-sibling::input").clear()
            browser.find_element_by_xpath("//*[@name='local_keyword']/preceding-sibling::input").send_keys()
            log.info("开启变量引用")
        else:
            # 目录树选择目录
            browser.find_element_by_xpath("//*[@name='srcPath']/preceding-sibling::input").click()
...
def file_load_mode_file_set(choose_type, file_name, file_type, encoding, begin_line, seperator, sheet_index,
    ...
    num = row_num
    # 文件配置
    focus_element = browser.find_element_by_xpath(
        "//*[@id='loadfilepath{0}']//*[contains(text(),'文件') and @class='requireTag']".format(num))
    ...
    if choose_type == "关键字":
        input_xpath = "//*[@name='filepath{0}']/preceding-sibling::input".format(num)
        form_selector = "//*[@id='file_load_form']"
        set_text_enable_var(input_xpath=input_xpath, form_selector=form_selector, msg=file_name)
        sleep(1)
    else:
        browser.find_element_by_xpath(
            "//*[@name='keyExpr{0}']/preceding-sibling::span/a".format(num)).click()
        sleep(1)
        # 切换到正则配置iframe页面
        browser.switch_to.frame(
            browser.find_element_by_xpath("//iframe[contains(@src,'operateWashRegexBox.html')]"))
    ...
    if sheet_index:
        browser.find_element_by_xpath(
            "//*[@name='sheetName{0}']/preceding-sibling::input".format(num)).send_keys(sheet_index)
    ...
    if begin_line:
        browser.find_element_by_xpath(
            "//*[@name='startRedLine{0}']/preceding-sibling::input".format(num)).send_keys(begin_line)
    ...
    # 设置变量
    browser.find_element_by_xpath(
        "//*[@id='fileVarName{0}']/following-sibling::span[1]/input[1]".format(num)).send_keys(var_name)
    ...
    if value_type:
        browser.find_element_by_xpath(
            "//*[@id='valueType{0}']/following-sibling::span[1]/input[1]".format(num)).click()
        browser.find_element_by_xpath(
            "//*[contains(@id,'valueType{0}') and text()='{1}']".format(num, value_type)).click()
    ...
    if filter_set:
        ...
        # 聚焦元素
        enable_filter_click = browser.find_element_by_xpath("//*[@id='isFilter{0}']".format(num))
        browser.execute_script("arguments[0].scrollIntoView(true);", enable_filter_click)
        if filter_set.get("flag") == "是":
            log.info("开启过滤")
            # 勾选开启过滤
            browser.find_element_by_xpath("//*[@id='isFilter{0}']".format(num)).click()
            sleep(1)
            confirm_selector = "//*[@id='isFilterIfr_{0}']".format(num)
            # 调用正则魔方配置正则
            regular_cube(confirm_selector=confirm_selector, set_type=filter_set.get("设置方式"),
