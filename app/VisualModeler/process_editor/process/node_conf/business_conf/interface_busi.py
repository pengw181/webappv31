...
def interface_business(node_name, interface, request_body, request_header, params, advance_set):
    ...
    # 选择接口
    if interface:
        browser.find_element_by_xpath("//*[@name='interfaceId']/preceding-sibling::input").click()
        interface_element = browser.find_element_by_xpath(
            "//*[contains(@id,'interfaceId') and text()='{0}']".format(interface))
    ...
    # 请求体内容
    if request_body:
        request_ele = browser.find_element_by_xpath("//*[@id='requestBody']")
        browser.execute_script("arguments[0].scrollIntoView(true);", request_ele)
        set_blob(textarea=request_ele, array=request_body)
        log.info("设置请求体内容")
        sleep(1)
    ...
    if request_header:
        log.info("开始配置请求头列表")
        request_header_title = browser.find_element_by_xpath("//*[@class='panel_title' and contains(text(),'请求头列表')]")
    ...
    if params:
        log.info("开始配置参数列表")
        params_title = browser.find_element_by_xpath("//*[@class='panel_title' and contains(text(),'参数列表')]")
    ...
    # 保存业务配置
    browser.find_element_by_xpath("//*[@onclick='savePortParamInfo(true)']//*[text()='保存']").click()
    log.info("保存业务配置")
...
def set_header(param_name, param_type, param_value):
    ...
    browser.find_element_by_xpath(
        req_header + "//*[text()='{0}']/../following-sibling::td[1][@field='valueType']//a".format(param_name)).click()
    val_type_element = browser.find_elements_by_xpath(
        req_header + "//*[contains(@id,'valType') and text()='{0}']".format(param_type))
    # 点击当前页面可见的元素
    ...
    if param_type == "变量":
        browser.find_element_by_xpath(
            req_header + "//*[text()='{0}']/../following-sibling::td[2][@field='paramValue']//a".format(param_name)).click()
        # 选择变量
        choose_var(var_name=param_value)
    else:
        # 输入固定值内容
        input_ele = browser.find_element_by_xpath(
            req_header + "//*[text()='{0}']/../following-sibling::td[2][@field='paramValue']//*[contains(@id,'input')]".format(
                param_name))
        input_ele.clear()
