...
def ocr_business(node_name, storage_set, filter_set, advance_set):
    """
    :param node_name: 节点名称
    :param storage_set: 存储参数配置，字典
    :param filter_set: 启用过滤配置，字典
    ...
    if storage_set:
        ...
        storage_param_set(storage_type=storage_set.get("存储类型"), dir_name=storage_set.get("目录"),
                          ftp=storage_set.get("远程服务器"), use_var=flag)
    ...
    if filter_set:
        ...
        # 聚焦元素
        enable_filter_element = browser.find_element_by_xpath("//*[@id='enableFilterCfg']")
                ...
                if choose_type == "0":
                    ...
                    if num > 1:
                        browser.find_element_by_xpath(
                            "//*[@id='fileType{0}']/../following-sibling::div/a[@onclick='addFilePath(this)']".format(
                                num - 1)).click()
                    break
        ...
        elif filter_set.get("flag") == "否":
            if status:
                enable_filter_element.click()
                log.info("关闭【启用过滤配置】")
            else:
                log.info("【启用过滤配置】标识为否，不开启")
        else:
            raise KeyError("【启用过滤配置: flag】 只支持输入：是/否，请检查")
    ...
    if advance_set:
        if advance_set.get("flag") == "是":
            ...
            try:
                enable_click = browser.find_element_by_xpath("//*[@onclick='toggleAdv($(this))']//*[text()='开启高级模式']")
                enable_click.click()
                log.info("开启【高级配置】")
            ...
            try:
                browser.find_element_by_xpath("//*[@onclick='toggleAdv($(this))']//*[text()='开启高级模式']")
            except NoSuchElementException:
                disable_click = browser.find_element_by_xpath("//*[@onclick='toggleAdv($(this))']//*[text()='关闭高级模式']")
                disable_click.click()
                log.info("关闭【高级配置】")
    ...
    # 保存业务配置
    browser.find_element_by_xpath("//*[@onclick='saveOcrNodeInfo(false)']//*[text()='保存']").click()
    log.info("保存业务配置")
    ...
    if storage_type == "本地":
        ...
        if use_var:
            ...
            if dir_name:
                ...
                browser.find_element_by_xpath("//*[@name='local_keyword']/preceding-sibling::input").send_keys(dir_name)
        ...
        log.info("配置远程ftp文件")
        browser.find_element_by_xpath("//*[@name='storageType' and @onclick=\"getStorageType(this,'1')\"]").click()
...
def file_filter(choose_type, file_name, file_type, row_num):
    ...
    if choose_type == "关键字":
        ...
        # 输入文件名
        if file_name:
            browser.find_element_by_xpath("//*[@name='keyExpr{0}']/preceding-sibling::span/a".format(num)).click()
            sleep(1)
            # 切换到正则配置iframe页面
    ...
    if file_type:
        browser.find_element_by_xpath(
            "//*[@name='fileType{0}']/preceding-sibling::input".format(num)).click()
        browser.find_element_by_xpath(
