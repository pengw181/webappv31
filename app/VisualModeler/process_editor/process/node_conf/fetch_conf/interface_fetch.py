...
def interface_fetch(opt, target_var, var_name, expression, value_type):
    """
    :param opt: 操作
    ...
    if opt == "添加":
        browser.find_element_by_xpath("//*[@onclick='addPortGetInfo()']//*[text()='添加']").click()
        ...
        if expression:
            ...
            browser.find_element_by_xpath("//*[@name='varExpr']/preceding-sibling::input").send_keys(expression)
            log.info("设置表达式: {0}".format(expression))
            sleep(1)
        ...
        # 点击保存
        browser.find_element_by_xpath("//*[@onclick='addPortDataRow()']//*[text()='保存']").click()
        ...
        sleep(1)
        browser.find_element_by_xpath("//*[@onclick='edit_getdata_port();']//*[text()='修改']").click()
        wait = WebDriverWait(browser, 30)
        wait.until(ec.element_to_be_clickable((By.XPATH, "//*[@name='varName']/preceding-sibling::input")))
        ...
        browser.find_element_by_xpath("//*[@field='varName']//*[text()='{0}']".format(target_var)).click()
        sleep(1)
        browser.find_element_by_xpath("//*[@onlick='del_getdata_port();']//*[text()='删除']").click()
    ...
    # 切换到取数配置iframe
    browser.switch_to.frame(browser.find_element_by_xpath("//iframe[contains(@src,'getDataScriptNode.html')]"))
