...
def cmd_fetch(opt, target_var, var_name, obj_type, result_type, cmd_name, temp_var, value_type):
    ...
    if opt == "添加":
        ...
        # 变量名
        if temp_var:
            browser.find_element_by_xpath("//*[@id='cmdVarId']/following-sibling::span/input[1]").click()
            browser.find_element_by_xpath(
                "//*[contains(@id,'cmdVarId') and text()='{0}']".format(temp_var)).click()
        ...
        # 切换到取数配置iframe
        browser.switch_to.frame(browser.find_element_by_xpath("//iframe[@id='getdata_cmd_node']"))
