...
def sort(input_var, sort_config, output_var, output_col, value_type):
    ...
    browser.switch_to.frame(
        browser.find_element_by_xpath("//iframe[contains(@src,'operateCfgSort.html')]"))

    # 选择变量
    if input_var:
        browser.find_element_by_xpath("//*[@id='dataH_inputSortVarName']/following-sibling::span//a").click()
    ...
    if output_var:
        ...
        if output_var_type == "输入":
            out_var_element = browser.find_element_by_xpath(
                "//*[@id='dataH_resultBaseVarName']/following-sibling::span//input[1]")
            ...
            # 输出名称选择变量
            browser.find_element_by_xpath("//*[@id='dataH_resultBaseVarName']/following-sibling::span//a").click()
    ...
    if output_col:
        ...
        browser.find_element_by_xpath("//*[@name='outColumn']/preceding-sibling::input").send_keys(output_col)
...
def sort_col(action, sorted_col_index, col_index, sort_type):
    ...
    # 操作
    if action == "添加":
        browser.find_element_by_xpath("//*[@onclick='addSortInfo();']//*[text()='添加']").click()
        sleep(1)

        # 列索引
        if col_index:
            browser.find_element_by_xpath("//*[@name='col_index']/preceding-sibling::input").send_keys(col_index)

        # 排序方式
        if sort_type:
            browser.find_element_by_xpath("//*[@name='sort_type']/preceding-sibling::input").click()
            browser.find_element_by_xpath(
                "//*[contains(@id,'sort_type') and text()='{0}']".format(sort_type)).click()

        browser.find_element_by_xpath("//*[@onclick='addSortRow()']//*[text()='保存']").click()
        log.info("添加排序列: {0}".format(col_index))

    elif action == "修改":
        browser.find_element_by_xpath(
            "//*[contains(@id,'varSortTable')]//div[contains(text(),'排序列：{0}')]".format(sorted_col_index)).click()
        browser.find_element_by_xpath("//*[@onclick='edit_sort_var();']//*[text()='修改']").click()

        # 列索引
        if col_index:
            browser.find_element_by_xpath("//*[@name='col_index']/preceding-sibling::input").clear()
            browser.find_element_by_xpath(
                "//*[@name='col_index']/preceding-sibling::input").send_keys(col_index)
        ...
        browser.find_element_by_xpath(
            "//*[contains(@id,'varSortTable')]//div[contains(text(),'排序列：{0}')]".format(sorted_col_index)).click()
        browser.find_element_by_xpath("//*[@onclick='delete_sort_var();']//*[text()='删除']").click()
        ...
        if alert.title_contains("确定需要删除", auto_click_ok=False):
            ...
            log.warn("删除排序列失败，失败提示: {0}".format(msg))
        ...
        # 切换到过滤运算iframe
        browser.switch_to.frame(browser.find_element_by_xpath("//iframe[contains(@src,'operateCfgSort.html')]"))
