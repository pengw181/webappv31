...
def network_addr(input_type, input_addr, ip, output_var, value_type):
    ...
    # 切换到网络地址运算iframe
    browser.switch_to.frame(browser.find_element_by_xpath("//iframe[contains(@src,'operateCfgIpaddr.html')]"))

    # 输入方式
    if input_type:
        browser.find_element_by_xpath("//*[text()='{0}']".format(input_type)).click()
    ...
    if input_addr:
        ...
        form_selector = "//*[@class='cfg_box']"
        set_text_enable_var(input_xpath=input_xpath, form_selector=form_selector, msg=input_addr)
    ...
    if ip:
        ...
        form_selector = "//*[@class='cfg_box']"
        set_text_enable_var(input_xpath=input_xpath, form_selector=form_selector, msg=ip)
