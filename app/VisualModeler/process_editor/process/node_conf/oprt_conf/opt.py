...
from app.VisualModeler.process_editor.process.node_conf.oprt_conf.calculation import CalculationCenter
from app.VisualModeler.process_editor.process.node_conf.oprt_conf.condition_loop import *
...
class OptTreeServer:
    ...
    def r_click_opt(self, obj, opt):
        ...
        wait.until(ec.visibility_of_element_located((By.XPATH, self.locator + "//*[contains(text(),'{0}')]".format(obj))))
        try:
            right_objs = self.browser.find_elements_by_xpath(self.locator + "//*[contains(text(),'{0}')]".format(obj))
        ...
        sleep(1)
        right_menu = self.browser.find_elements_by_xpath("//*[text()='{0}']".format(opt))
    ...
    def add_if(self, if_array, enable_else):
        ...
        if self.common_tree:
            self.browser.switch_to.frame(
                self.browser.find_element_by_xpath("//iframe[contains(@src,'operateNodeCondition.html')]"))
        else:
            self.browser.switch_to.frame(
                self.browser.find_element_by_xpath("//iframe[contains(@src,'crawlerOperateCondition.html')]"))

        # 点击修改按钮
        if self.common_tree:
            self.browser.find_element_by_xpath("//*[@onclick=\"showAdd('oprtfitcnd');\"]").click()
        else:
            self.browser.find_element_by_xpath("//*[@onclick=\"showAdd('oprtfitcnd','1');\"]").click()
        ...
        if self.common_tree:
            enable_click = self.browser.find_element_by_xpath("//*[@id='is_cnd_else']")
        else:
            enable_click = self.browser.find_element_by_xpath("//*[@for='is_cnd_else']")
        ...
        # 保存
        if self.common_tree:
            self.browser.find_element_by_xpath("//*[@onclick='saveCondtion()']//*[text()='保存']").click()
        else:
            self.browser.find_element_by_xpath("//*[@onclick='saveFetchCondtions()']//*[text()='保存']").click()
        ...
        if self.common_tree:
            ...
            self.browser.switch_to.frame(self.browser.find_element_by_xpath(get_global_var("NodeIframe")))
            # 切换到业务配置iframe
            self.browser.switch_to.frame(self.browser.find_element_by_xpath(get_global_var("CrawlerIframe")))
    ...
    def add_loop(self, where, loop_type, loop_info):
        ...
        # 切换到循环配置iframe
        self.browser.switch_to.frame(self.browser.find_element_by_xpath(option_iframe_xpath))
        ...
        # 循环类型
        if loop_type:
            elements = self.browser.find_elements_by_xpath("//*[@name='looptype']/..//*[text()='{0}']".format(loop_type))
            for e in elements:
                if e.is_displayed():
                    e.click()
                    sleep(1)
        ...
        # 保存循环
        if self.common_tree:
            self.browser.find_element_by_xpath("//*[@onclick='saveCircleInfo()']//*[text()='保存']").click()
        else:
            self.browser.find_element_by_xpath("//*[@onclick='saveFetchCircleInfos()']//*[text()='保存']").click()
        ...
        if alert.title_contains("成功"):
            ...
            log.warn("保存循环失败，失败提示: {0}".format(msg))
        set_global_var("ResultMsg", msg, False)
        ...
        if where == 1:
            ...
            self.browser.switch_to.frame(self.browser.find_element_by_xpath(get_global_var("NodeIframe")))
            # 切换到控制配置iframe
            self.browser.switch_to.frame(self.browser.find_element_by_xpath(get_global_var("ControlIframe")))
        else:
            # 切换到节点iframe
            self.browser.switch_to.frame(self.browser.find_element_by_xpath(get_global_var("NodeIframe")))
            # 切换到操作配置iframe
    ...
    def add_step(self, steps):
        ...
        # 切换到选择步骤iframe
        self.browser.switch_to.frame(
            self.browser.find_element_by_xpath("//iframe[contains(@src,'crawlerOperateSteps.html')]"))
        sleep(1)

        for s in steps:
            self.browser.find_element_by_xpath("//*[@name='stepName']/preceding-sibling::input").clear()
            self.browser.find_element_by_xpath("//*[@name='stepName']/preceding-sibling::input").send_keys(s)
            self.browser.find_element_by_xpath("//*[@data-dg-query='#query_steps_tab']//*[text()='查询']").click()
            sleep(2)
            page_wait()
            self.browser.find_element_by_xpath(
                "//*[contains(@id,'query_steps')]//*[@field='elementName']//*[text()='{0}']".format(s)).click()
            self.browser.find_element_by_xpath("//*[@onclick='saveChooseStep();']//*[text()='保存']").click()
            ...
            if alert.title_contains("操作成功"):
                ...
                log.warn("添加步骤失败，失败提示: {0}".format(msg))
            set_global_var("ResultMsg", msg, False)

            # 切换到节点iframe
            self.browser.switch_to.frame(
                self.browser.find_element_by_xpath("//iframe[contains(@src,'./node/crawlerNode.html?')]"))
            # 切换到业务配置iframe
            self.browser.switch_to.frame(
                self.browser.find_element_by_xpath("//iframe[@id='busi_crawler_node']"))
            # 切换到选择步骤iframe
            self.browser.switch_to.frame(
                self.browser.find_element_by_xpath("//iframe[contains(@src,'crawlerOperateSteps.html')]"))
        # 添加步骤完成后，返回上层iframe
    ...
    def bind_subprocess(self, subprocess_name, create_begin_time, create_end_time, cond=None):
        """
        ...
        # 切换到子流程iframe
        self.browser.switch_to.frame(
            self.browser.find_element_by_xpath("//iframe[contains(@src,'operateNodeChild.html')]"))

        # 点击绑定子流程
        self.browser.find_element_by_xpath(
            "//*[@id='addQuickdiv']/*[@onclick='addSubProcess();']/*[text()='绑定子流程']").click()
        page_wait()
        sleep(1)

        # 进入操作子流程iframe
        self.browser.switch_to.frame(
            self.browser.find_element_by_xpath("//iframe[contains(@src,'operateSubprocess.html')]"))
        ...
        if subprocess_name:
            self.browser.find_element_by_xpath(
                "//*[@name='keyword']/preceding-sibling::input").send_keys(subprocess_name)
            log.info("设置子流程名称: {0}".format(subprocess_name))
            sleep(1)

        # 创建开始时间
        if create_begin_time:
            self.browser.find_element_by_xpath("//*[@name='createTime']/preceding-sibling::span[1]/a").click()
            log.info("设置创建开始时间: {0}".format(create_begin_time))
            set_date(date_s=create_begin_time)
            sleep(1)

        # 创建结束时间
        if create_end_time:
            self.browser.find_element_by_xpath("//*[@name='endTime']/preceding-sibling::span[1]/a").click()
            log.info("设置创建结束时间: {0}".format(create_end_time))
            set_date(date_s=create_end_time)
            sleep(1)

        # 查询
        self.browser.find_element_by_xpath("//*[@data-dg-query='#process_info_tab']//*[text()='查询']").click()
        page_wait()

        # 选择子流程
        self.browser.find_element_by_xpath(
            "//*[contains(@id,'process_info_tab')]/*[@field='processName']//*[text()='{0}']".format(
                subprocess_name)).click()
        log.info("选择子流程: {0}".format(subprocess_name))

        # 点击保存
        self.browser.find_element_by_xpath("//*[@id='save_process_button']//*[text()='保存']").click()
        alert = BeAlertBox(back_iframe="default")
        msg = alert.get_msg()
        if alert.title_contains("成功"):
            ...
            log.warn("子流程 {0} 绑定失败，失败提示: {1}".format(subprocess_name, msg))
            result = False
        set_global_var("ResultMsg", msg, False)
        ...
        if result:
            ...
            if cond:
                ...
                self.browser.find_element_by_xpath(
                    "//*[@onclick=\"showAdd('subProCnd');\"]//*[text()='修改']").click()
                log.info("绑定子流程设置条件")
    ...
    def add_subprocess(self, process_name, field, process_type, process_desc, except_abort, cond=None):
        """
        ...
        # 点击添加子流程
        self.browser.find_element_by_xpath(
            "//*[@id='addQuickdiv']/*[@onclick='createChildProcess();']/*[text()='添加子流程']").click()
        sleep(1)

        # 进入添加子流程iframe
        self.browser.switch_to.frame(
            self.browser.find_element_by_xpath("//iframe[contains(@src,'../processInfoEdit.html')]"))
        ...
        if field:
            ...
            for f in field:
                self.browser.find_element_by_xpath(
                    "//*[contains(@id,'temp_type_id') and text()='{0}']".format(f)).click()
            self.browser.find_element_by_xpath(
                "//*[contains(text(),'专业领域')]/../following-sibling::div[1]/div/span").click()
        ...
        if except_abort:
            ...
            self.browser.find_element_by_xpath("//*[@id='isNodeExpEnd']").click()
            log.info("关闭【节点异常终止流程】")

        # 点击提交
        self.browser.find_element_by_xpath("//*[contains(@href,'submitForm()')]//*[text()='提交']").click()
        alert = BeAlertBox(back_iframe="default")
        msg = alert.get_msg()
        if alert.title_contains("成功"):
            ...
            log.warn("子流程 {0} 添加失败，失败提示: {1}".format(process_name, msg))
            result = False
        set_global_var("ResultMsg", msg, False)
    ...
    def copy(self):
        ...
        if alert.title_contains("成功"):
            ...
            log.warn("操作复制失败，失败提示: {0}".format(msg))
        set_global_var("ResultMsg", msg, False)
    ...
    def delete(self):
        ...
        if alert.title_contains("您确定需要删除", auto_click_ok=False):
            ...
            if alert.title_contains("成功"):
                ...
                log.warn("步骤从操作树删除失败，失败提示: {0}".format(msg))
                result = False
            set_global_var("ResultMsg", msg, False)
        else:
            log.warn("步骤从操作树删除失败，失败提示: {0}".format(msg))
