...
from app.VisualModeler.process_editor.process.node_conf.oprt_conf.condition_loop import condition
from common.page.func.alert_box import BeAlertBox
from app.VisualModeler.process_editor.process.node_conf.oprt_conf.cal.regular import regular
from app.VisualModeler.process_editor.process.node_conf.oprt_conf.cal.basic import basic
from app.VisualModeler.process_editor.process.node_conf.oprt_conf.cal.filter import filter
from app.VisualModeler.process_editor.process.node_conf.oprt_conf.cal.aggregate import aggregate
from app.VisualModeler.process_editor.process.node_conf.oprt_conf.cal.network_addr import network_addr
from app.VisualModeler.process_editor.process.node_conf.oprt_conf.cal.section import section
from app.VisualModeler.process_editor.process.node_conf.oprt_conf.cal.sort import sort
from app.VisualModeler.process_editor.process.node_conf.oprt_conf.cal.wash import wash
...
class CalculationCenter:
    ...
    def __init__(self, oprt_type):
        ...
        # 选择运算类型
        self.browser.find_element_by_xpath("//*[@name='operate_type']/preceding-sibling::input").click()
        self.browser.find_element_by_xpath(
            "//*[contains(@id,'oprt_type') and text()='{0}']".format(self.oprt_type)).click()
    ...
    def cal_condition(self, array):
        ...
        """
        self.browser.find_element_by_xpath("//*[@onclick=\"showAdd('oprtfitcnd','1');\"]//*[text()='修改']").click()
        condition(array=array)

    def cal_save(self):

        # 点击保存
        save_elements = self.browser.find_elements_by_xpath("//*[@onclick='saveOprtResult();']//*[text()='保存']")
        ...
        if alert.title_contains("操作成功"):
            ...
            log.warn("保存运算失败，失败提示: {0}".format(msg))
    ...
    def movement(self, expression):
        ...
        self.browser.switch_to.frame(
            self.browser.find_element_by_xpath("//iframe[contains(@src,'operateCfgAction.html')]"))
