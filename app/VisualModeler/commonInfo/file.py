...
class File:
    ...
    def __init__(self, catalog):
        ...
        if catalog == "personal":
            ...
            self.browser.switch_to.frame(self.browser.find_element_by_xpath(
                "//iframe[contains(@src, '/VisualModeler/html/commonInfo/catalogPersonalDef.html')]"))
            log.info("进入个人目录")
        elif catalog == "system":
            dw.choose_menu("常用信息管理-文件目录管理-系统目录")
            self.browser.switch_to.frame(self.browser.find_element_by_xpath(
                "//iframe[contains(@src, '/VisualModeler/html/commonInfo/catalogSystemDef.html')]"))
            log.info("进入系统目录")
        else:
    ...
    def mkdir(self, parent_dir, dir_name):
        ...
        add_elements = self.browser.find_elements_by_xpath("//*[text()='添加目录']")
        ...
        if parent_dir != cur_parent:
            ...
            # 输入目录名称
            self.browser.find_element_by_xpath("//*[@name='pathName']/preceding-sibling::input").send_keys(dir_name)
            self.browser.find_element_by_xpath("//*[@onclick='addPathSave()']//*[text()='提交']").click()
            ...
            if alert.title_contains("操作成功"):
                ...
                log.warn("目录 {0} 添加失败，失败提示: {1}".format(dir_name, msg))
    ...
    def update_dir(self, target_dir, new_dir):
        ...
        update_elements = self.browser.find_elements_by_xpath("//*[text()='修改目录']")
        ...
        try:
            ...
            # 输入目录名称
            self.browser.find_element_by_xpath("//*[@name='pathName']/preceding-sibling::input").clear()
            self.browser.find_element_by_xpath("//*[@name='pathName']/preceding-sibling::input").send_keys(new_dir)
            self.browser.find_element_by_xpath("//*[@onclick='updPathSave()']//*[text()='提交']").click()
            ...
            if alert.title_contains("成功"):
                ...
                log.warn("目录 {0} 修改失败，失败提示: {1}".format(target_dir, msg))
            set_global_var("ResultMsg", msg, False)
        except TimeoutError:
    ...
    def delete_dir(self, dir_name):
        ...
        delete_elements = self.browser.find_elements_by_xpath("//*[text()='删除目录']")
        dir_exist = False
            ...
            if element.is_displayed():
                ...
                if alert.title_contains(dir_name, auto_click_ok=False):
                    ...
                    if alert.title_contains("成功"):
                        ...
                        log.warn("目录{0} 删除失败，失败提示: {1}".format(dir_name, msg))
                else:
                    log.warn("目录{0} 删除失败，失败提示: {1}".format(dir_name, msg))
                set_global_var("ResultMsg", msg, False)
        return dir_exist
    ...
    def upload_file(self, dir_name, file_name):
        ...
        self.browser.find_element_by_xpath("//*[@id='uploadBtn']//*[text()='上传文件']").click()
        wait = WebDriverWait(self.browser, 30)
        wait.until(ec.frame_to_be_available_and_switch_to_it((
        ...
        if alert.title_contains("上传成功"):
            ...
            sleep(1)
            files = self.browser.find_elements_by_xpath(
                "//*[@value='{0}']/..//*[contains(@id,'catalogDef_info_tab_')][1]/*[@field='fileName']/div".format(
                    self.catalog))
            for f in files:
                if f.is_displayed():
            ...
            log.warn("目录【{0}】上传文件 {1} 失败，失败提示: {2}".format(dir_name, file_name, msg))
        set_global_var("ResultMsg", msg, False)
    ...
    def choose_file(self, dir_name, file_name):
        ...
        self.browser.find_element_by_xpath(
            "//*[@value='{0}']/..//*[@name='fileName']/preceding-sibling::input".format(
                self.catalog)).send_keys(file_name)
        self.browser.find_element_by_xpath(
            "//*[@value='{0}']/..//*[@id='queryBtn']//*[text()='查询']".format(self.catalog)).click()
        sleep(1)
    ...
    def download_file(self, dir_name, file_name):
        ...
        self.browser.find_element_by_xpath(
            "//*[@value='{0}']/..//*[@funcid='systemFile_down' and contains(@href,'{1}')]".format(
    ...
    def download_file_batch(self, dir_name, file_names):
        ...
        for file in file_names:
            try:
                self.browser.find_element_by_xpath(
                    "//*[@value='{0}']/..//*[contains(@id,'catalogDef')]/*[@field='fileName']//*[text()='{1}']".format(
                        self.catalog, file)).click()
                log.info("选择文件: {0}".format(file))
                sleep(1)
            except NoSuchElementException:
                log.warn("文件: {0}不在第一页列表中".format(file))
                raise
        self.browser.find_element_by_xpath("//*[@id='downloadBtn']//*[text()='批量下载']").click()
        log.info("点击批量下载")
    ...
    def delete_file(self, dir_name, file_name):
        ...
        # 点击删除文件
        self.browser.find_element_by_xpath(
            "//*[@value='{0}']/..//*[@funcid='systemFile_del' and contains(@href,'{1}')]".format(
                self.catalog, file_name)).click()
        log.info("点击删除文件")
        ...
        if alert.title_contains("您确定需要删除{0}吗".format(file_name), auto_click_ok=False):
            ...
            if alert.title_contains("成功"):
                ...
                log.warn("目录【{0}】删除文件: {1} 失败，返回结果: {2}".format(dir_name, file_name, msg))
        else:
            log.warn("目录【{0}】删除文件: {1} 失败，返回结果: {2}".format(dir_name, file_name, msg))
        set_global_var("ResultMsg", msg, False)
    ...
    def delete_file_batch(self, dir_name, file_names):
        ...
        for file in file_names:
            try:
                ...
                log.warn("文件: {0}不在第一页列表中".format(file))
                raise
        self.browser.find_element_by_xpath("//*[@id='deleteBtn']//*[text()='批量删除']").click()
        log.info("点击批量删除")
        ...
        if alert.title_contains("您确定需要批量删除吗", auto_click_ok=False):
            ...
            if alert.title_contains("成功"):
                ...
                log.warn("目录【{0}】删除文件: {1} 失败，返回结果: {2}".format(dir_name, ",".join(file_names), msg))
        else:
            log.warn("目录【{0}】删除文件: {1} 失败，返回结果: {2}".format(dir_name, ",".join(file_names), msg))
        set_global_var("ResultMsg", msg, False)
    ...
    def data_clear(self, obj):
        ...
        dir_elements = self.browser.find_elements_by_xpath(
            "//*[@class='tree-title' and text()='{0}']".format(obj))
