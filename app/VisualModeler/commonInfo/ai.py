...
class AiModel:
    ...
    def choose(self, model_name):
        ...
        self.browser.find_element_by_xpath(
            "//*[contains(@id,'templetManage')]/*[@field='modelName']/*[text()='{0}']".format(model_name)).click()
    ...
    def algorithm_page(self, application_mode, algorithm, model_name, model_desc, train_scale, test_scale, timeout,
        ...
        # 模型描述
        if model_desc:
            model_desc_textarea = self.browser.find_element_by_xpath("//*[@name='modelDesc']/preceding-sibling::textarea")
            set_textarea(textarea=model_desc_textarea, msg=model_desc)
            log.info("设置模型描述: {0}".format(model_desc))

        # 训练比例
        if train_scale:
            self.browser.find_element_by_xpath("//*[@id='trainDataScale']/following-sibling::span//input[1]").clear()
            self.browser.find_element_by_xpath(
                "//*[@id='trainDataScale']/following-sibling::span//input[1]").send_keys(train_scale)
            log.info("设置训练比例: {0}".format(train_scale))

        # 测试比例
        if test_scale:
            self.browser.find_element_by_xpath("//*[@id='testDataScale']/following-sibling::span//input[1]").clear()
            self.browser.find_element_by_xpath(
                "//*[@id='testDataScale']/following-sibling::span//input[1]").send_keys(test_scale)
            log.info("设置测试比例: {0}".format(test_scale))

        # 超时时间
        if timeout:
            self.browser.find_element_by_xpath(
                "//*[@id='train_test_timeout']/following-sibling::span/input[1]").clear()
            self.browser.find_element_by_xpath(
                "//*[@id='train_test_timeout']/following-sibling::span/input[1]").send_keys(timeout)
            log.info("设置超时时间: {0}".format(timeout))

        # 模型数据
        if file_name:
            # 点击选择文件
            # self.browser.find_element_by_xpath("//*[@id='fileName']/following-sibling::span//label").click()
            # sleep(1)
            log.info("开始上传模型数据文件: {0}".format(file_name))
            upload(file_name=file_name, catalog="model", input_id="filebox_file_id_3")
            sleep(1)
            # 点击上传按钮
            self.browser.find_element_by_xpath("//*[@onclick='importAITestFile()']//*[text()='上传']").click()
            ...
            if alert.title_contains("文件上传成功"):
                ...
                log.warn("{0} 上传失败，失败提示: {1}".format(file_name, msg))
            set_global_var("ResultMsg", msg, False)

        # 参数设置
        if params:
            algorithm_obj = self.browser.find_element_by_xpath("//*[@name='algorithm']")
        ...
        # 提交
        self.browser.find_element_by_xpath("//*[@id='saveButton']//*[text()='提交']").click()
    ...
    def ai_param_set(self, algorithm, params):
        ...
        for param in params:
            ...
            self.browser.find_element_by_xpath(
                "//*[@id='{0}']/following-sibling::span/input[1]".format(param_tag)).clear()
            self.browser.find_element_by_xpath(
                "//*[@id='{0}']/following-sibling::span/input[1]".format(param_tag)).send_keys(param)
    ...
    def ai_col_set(self, algorithm, col_sets):
        ...
        for col_name, col_value in col_sets.items():
            ...
            if col_value:
                # 点开下拉框
                self.browser.find_element_by_xpath("//*[@id='{0}']/following-sibling::span//a".format(col_tag)).click()
                sleep(1)

                # 时间列、预测列单选
                if col_name in ["时间列", "预测列"]:
                    col_ele = self.browser.find_elements_by_xpath(
                        "//*[@class='tree-title' and text()='{0}']".format(col_value))
                    if len(col_ele) > 0:
                    ...
                    for key in col_value:
                        col_ele = self.browser.find_elements_by_xpath(
                            "//*[@class='tree-title' and text()='{0}']".format(key))
                        if len(col_ele) > 0:
                            for element in col_ele:
                                if element.is_displayed():
                                    element.click()
                    ...
                    self.browser.find_element_by_xpath(
                        "//*[@id='{0}']/following-sibling::span//a".format(col_tag)).click()
                num += 1
            else:
                num += 1
    ...
    def train_model(self, model_name):
        ...
        if alert.exist_alert:
            ...
            # 点击训练按钮
            self.browser.find_element_by_xpath("//*[@onclick='getTrainResult()']").click()
            ...
            if alert.title_contains("提交训练成功"):
                ...
                log.warn("{0} 提交训练失败，失败提示: {1}".format(model_name, msg))
            set_global_var("ResultMsg", msg, False)
    ...
    def test_model(self, model_name):
        ...
        if alert.exist_alert:
            ...
            # 点击测试按钮
            self.browser.find_element_by_xpath("//*[@onclick='getTestResult()']").click()
            alert = BeAlertBox(timeout=30)
            msg = alert.get_msg()
            if alert.title_contains("提交测试成功"):
                ...
                log.warn("{0} 提交测试失败，失败提示: {1}".format(model_name, msg))
            set_global_var("ResultMsg", msg, False)
    ...
    def import_disturb(self, model_name, file_name):
        ...
        self.browser.find_element_by_xpath(
            "//*[text()='{0}']/../following-sibling::td[3]//a".format(model_name)).click()
        page_wait(10)
        sleep(1)
        wait = WebDriverWait(self.browser, 30)
        wait.until(ec.frame_to_be_available_and_switch_to_it((
        ...
        # 点击导入
        self.browser.find_element_by_xpath("//*[@onclick='toBatchAddOperate()']").click()
        sleep(1)
        wait = WebDriverWait(self.browser, 30)
        wait.until(ec.frame_to_be_available_and_switch_to_it((
        ...
        # 点击上传
        self.browser.find_element_by_xpath("//*[@onclick='uploadDisturbFile()']").click()
        ...
        if alert.title_contains("操作成功"):
            ...
            log.warn("{0} 导入失败，失败提示: {1}".format(file_name, msg))
        set_global_var("ResultMsg", msg, False)

    def data_clear(self, obj, fuzzy_match=False):
        # 用于清除数据，在测试之前执行, 使用关键字开头模糊查询
        self.browser.find_element_by_xpath("//*[@name='modelName']/preceding-sibling::input").clear()
        self.browser.find_element_by_xpath("//*[@name='modelName']/preceding-sibling::input").send_keys(obj)
        self.browser.find_element_by_xpath("//*[@id='queryButton']//*[text()='查询']").click()
        page_wait()
        fuzzy_match = True if fuzzy_match == "是" else False
        if fuzzy_match:
            record_element = self.browser.find_elements_by_xpath(
                "//*[@field='modelName']/*[starts-with(text(),'{0}')]".format(obj))
        else:
            record_element = self.browser.find_elements_by_xpath(
                "//*[@field='modelName']/*[text()='{0}']".format(obj))
