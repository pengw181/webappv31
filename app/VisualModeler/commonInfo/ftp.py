...
class FTP:
    ...
    def __init__(self):
        ...
        self.browser.switch_to.frame(self.browser.find_element_by_xpath(
            "//iframe[contains(@src, '/VisualModeler/html/commonInfo/ftpServerCfg.html')]"))
        page_wait()
        sleep(1)

    def choose(self, ftp_name):
        """
        :param ftp_name: 服务器名称
        """
        self.browser.find_element_by_xpath("//*[@name='serverName']/preceding-sibling::input").send_keys(ftp_name)
        self.browser.find_element_by_xpath("//*[@id='btn']//*[text()='查询']").click()
        page_wait()
        self.browser.find_element_by_xpath(
            "//*[contains(@id,'ftpServerCfg_info_tab_')]//*[text()='{0}']".format(ftp_name)).click()
        log.info("已选择: {0}".format(ftp_name))
    ...
    def add(self, server_name, ip, port, username, pwd, server_type, encoding, data_type):
        ...
        if alert.title_contains("成功"):
            ...
            log.warn("数据 {0} 添加失败，失败提示: {1}".format(server_name, msg))
        set_global_var("ResultMsg", msg, False)

    def update(self, obj, server_name, ip, port, username, pwd, server_type, encoding, data_type):
        """
        :param obj: 服务器名称
        :param server_name: 服务器名称
        :param ip: 服务器IP
    ...
    def ftp_page(self, server_name, ip, port, username, pwd, server_type, encoding, data_type):
        ...
        # 服务器IP
        if ip:
            self.browser.find_element_by_xpath("//*[@name='serverIp']/preceding-sibling::input").clear()
            self.browser.find_element_by_xpath("//*[@name='serverIp']/preceding-sibling::input").send_keys(ip)
            log.info("设置服务器IP: {0}".format(ip))

        # 服务器端口
        if port:
            self.browser.find_element_by_xpath("//*[@name='serverPort']/preceding-sibling::input").clear()
            self.browser.find_element_by_xpath("//*[@name='serverPort']/preceding-sibling::input").send_keys(port)
            log.info("设置服务器端口: {0}".format(port))

        # 用户名
        if username:
            self.browser.find_element_by_xpath("//*[@name='serverUser']/preceding-sibling::input").clear()
            self.browser.find_element_by_xpath("//*[@name='serverUser']/preceding-sibling::input").send_keys(username)
        ...
        if pwd:
            try:
                # 判断是否是修改密码
                self.browser.find_element_by_xpath(
                    "//*[@id='serverPwd']/following-sibling::span//a[contains(@class, 'edit')]").click()
            except NoSuchElementException:
                pass
            self.browser.find_element_by_xpath("//*[@name='serverPwd']/preceding-sibling::input").send_keys(pwd)
            sleep(1)
            log.info("设置密码: {0}".format(pwd))

        # 服务器类型
        if server_type:
            self.browser.find_element_by_xpath("//*[@name='serverType']/preceding-sibling::input").click()
            self.browser.find_element_by_xpath(
                "//*[contains(@id,'serverType') and text()='{0}']".format(server_type)).click()
            log.info("设置服务器类型: {0}".format(server_type))

        # 服务器编码
        if encoding:
            self.browser.find_element_by_xpath("//*[@name='serverEncoding']/preceding-sibling::input").click()
            self.browser.find_element_by_xpath(
                "//*[contains(@id,'serverEncoding') and text()='{0}']".format(encoding)).click()
    ...
    def data_clear(self, obj, fuzzy_match=False):
        ...
        self.browser.find_element_by_xpath("//*[@name='serverName']/preceding-sibling::input").clear()
        self.browser.find_element_by_xpath("//*[@name='serverName']/preceding-sibling::input").send_keys(obj)
        self.browser.find_element_by_xpath("//*[@id='btn']//*[text()='查询']").click()
        page_wait()
        fuzzy_match = True if fuzzy_match == "是" else False
        if fuzzy_match:
            record_element = self.browser.find_elements_by_xpath(
                "//*[@field='serverName']/*[contains(@class,'serverName') and starts-with(text(),'{0}')]".format(obj))
        else:
            record_element = self.browser.find_elements_by_xpath(
                "//*[@field='serverName']/*[contains(@class,'serverName') and text()='{0}']".format(obj))
                ...
                if alert.title_contains("您确定需要删除{0}吗".format(search_result), auto_click_ok=False):
                    ...
                    if alert.title_contains("成功"):
                        ...
                        if fuzzy_match:
                            # 重新获取页面查询结果
                            record_element = self.browser.find_elements_by_xpath(
                                "//*[@field='serverName']/*[contains(@class,'serverName') and starts-with(text(),'{0}')]".format(
                                    obj))
                            if len(record_element) > 0:
                                exist_data = True
                            else:
