...
class Proxy:
    ...
    def __init__(self):
        ...
        self.browser.switch_to.frame(self.browser.find_element_by_xpath(
            "//iframe[contains(@src, '/VisualModeler/html/commonInfo/proxyCfg.html')]"))
        page_wait()
        sleep(1)
    ...
    def search(self, proxy_name, ip_port, enable):
        ...
        # 代理名称
        if proxy_name:
            self.browser.find_element_by_xpath("//*[@name='proxyName']/preceding-sibling::input").send_keys(proxy_name)
            log.info("设置代理服务器/端口")

        # 代理服务器/端口
        if ip_port:
            self.browser.find_element_by_xpath("//*[@name='proxyIP']/preceding-sibling::input").send_keys(ip_port)
            log.info("设置代理服务器/端口")

        # 是否有效
        if enable:
            self.browser.find_element_by_xpath("//*[@name='isAlive']/preceding-sibling::input").click()
            self.browser.find_element_by_xpath("//*[contains(@id,'_combobox_') and text()='{0}']".format(enable)).click()
            log.info("设置是否有效")
    ...
    def choose(self, proxy_name):
        ...
        self.browser.find_element_by_xpath("//*[@id='btn']//*[text()='查询']").click()
        page_wait()
        self.browser.find_element_by_xpath("//*[contains(@id,'proxyCfg')]//*[text()='{0}']".format(proxy_name)).click()
    ...
    def add(self, proxy_name, ip, port, username, pwd, protocol, enable, data_type):
        ...
        if alert.title_contains("保存成功"):
            ...
            log.warn("{0} 添加失败，失败原因：{1}".format(proxy_name, msg))
    ...
    def update(self, obj, proxy_name, ip, port, username, pwd, protocol, enable, data_type):
        ...
        if exist:
            ...
            if alert.title_contains("成功"):
                ...
                log.warn("{0} 修改失败，失败原因：{1}".format(proxy_name, msg))
            set_global_var("ResultMsg", msg, False)
    ...
    def proxy_page(self, proxy_name, ip, port, username, pwd, protocol, enable, data_type):
        ...
        # 代理名称
        if proxy_name:
            self.browser.find_element_by_xpath("//*[@name='proxyName']/preceding-sibling::input").clear()
            self.browser.find_element_by_xpath("//*[@name='proxyName']/preceding-sibling::input").send_keys(proxy_name)
            log.info("设置代理名称: {0}".format(proxy_name))

        # 代理服务器
        if ip:
            self.browser.find_element_by_xpath("//*[@name='proxyIp']/preceding-sibling::input").clear()
            self.browser.find_element_by_xpath("//*[@name='proxyIp']/preceding-sibling::input").send_keys(ip)
            log.info("设置代理名称: {0}".format(ip))

        # 代理端口
        if port:
            self.browser.find_element_by_xpath("//*[@name='proxyPort']/preceding-sibling::input").clear()
            self.browser.find_element_by_xpath("//*[@name='proxyPort']/preceding-sibling::input").send_keys(port)
            log.info("设置代理端口: {0}".format(port))

        # 代理用户名
        if username:
            self.browser.find_element_by_xpath("//*[@name='proxyUser']/preceding-sibling::input").clear()
            self.browser.find_element_by_xpath("//*[@name='proxyUser']/preceding-sibling::input").send_keys(username)
        ...
        if pwd:
            try:
                ...
                self.browser.find_element_by_xpath(
                    "//*[@id='proxyPwd']/following-sibling::span//a[contains(@class, 'edit')]").click()
            except NoSuchElementException:
                pass
            self.browser.find_element_by_xpath("//*[@name='proxyPwd']/preceding-sibling::input").send_keys(pwd)
            sleep(1)
            log.info("设置代理密码: {0}".format(pwd))

        # 代理协议
        if protocol:
            self.browser.find_element_by_xpath("//*[@name='proxyType']/preceding-sibling::input").click()
            self.browser.find_element_by_xpath(
                "//*[contains(@id,'_combobox_') and text()='{0}']".format(protocol)).click()
        ...
        # 聚焦元素
        enable_click = self.browser.find_element_by_xpath("//*[@for='isAlive']")
        ...
        if data_type:
            ...
            self.browser.find_element_by_xpath(
                "//*[contains(@id,'dataTypeId_') and text()='{0}']".format(data_type)).click()
    ...
    def data_clear(self, obj, fuzzy_match=False):
        ...
        self.browser.find_element_by_xpath("//*[@name='proxyName']/preceding-sibling::input").clear()
        self.browser.find_element_by_xpath("//*[@name='proxyName']/preceding-sibling::input").send_keys(obj)
        self.browser.find_element_by_xpath("//*[@id='btn']//*[text()='查询']").click()
        page_wait()
        fuzzy_match = True if fuzzy_match == "是" else False
        if fuzzy_match:
            record_element = self.browser.find_elements_by_xpath(
                "//*[@field='proxyName']/*[contains(@class,'proxyName') and starts-with(text(),'{0}')]".format(obj))
        else:
            record_element = self.browser.find_elements_by_xpath(
                "//*[@field='proxyName']/*[contains(@class,'proxyName') and text()='{0}']".format(obj))
                ...
                if alert.title_contains("您确定需要删除{0}吗".format(search_result), auto_click_ok=False):
                    ...
                    if alert.title_contains("成功"):
                        ...
                        if fuzzy_match:
                            # 重新获取页面查询结果
                            record_element = self.browser.find_elements_by_xpath(
                                "//*[@field='proxyName']/*[contains(@class,'proxyName') and starts-with(text(),'{0}')]".format(
                                    obj))
                            if len(record_element) > 0:
                                exist_data = True
                            else:
