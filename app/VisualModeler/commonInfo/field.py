...
class ProfessionField:
    ...
    def __init__(self):
        ...
        self.browser.switch_to.frame(self.browser.find_element_by_xpath(
            "//iframe[contains(@src, '/VisualModeler/html/commonInfo/templetManageInfo.html')]"))
        page_wait()
        sleep(1)
    ...
    def choose(self, field_name):
        ...
        """
        self.browser.find_element_by_xpath("//*[@name='tempTypeName']/preceding-sibling::input").send_keys(field_name)
        self.browser.find_element_by_xpath("//*[@id='btn']//*[text()='查询']").click()
        page_wait()
        self.browser.find_element_by_xpath(
            "//*[contains(@id,'templetManage_info_tab_')]//*[text()='{0}']".format(field_name)).click()
    ...
    def add(self, field_name):
        ...
        if alert.title_contains("保存成功"):
            ...
            log.warn("{0} 添加失败，失败提示: {1}".format(field_name, msg))
    ...
    def data_clear(self, obj, fuzzy_match=False):
        ...
        if fuzzy_match:
            ...
            record_element = self.browser.find_elements_by_xpath(
                "//*[@field='tempTypeName']/*[contains(@class,'tempTypeName') and text()='{0}']".format(obj))
                ...
                if alert.title_contains("您确定需要删除{0}吗".format(search_result), auto_click_ok=False):
                    ...
                    if alert.title_contains("成功"):
                        ...
                        if fuzzy_match:
                            # 重新获取页面查询结果
                            record_element = self.browser.find_elements_by_xpath(
                                "//*[@field='tempTypeName']/*[contains(@class,'tempTypeName') and starts-with(text(),'{0}')]".format(
                                    obj))
                            if len(record_element) > 0:
                                exist_data = True
                            else:
