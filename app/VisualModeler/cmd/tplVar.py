...
class VarEditor:
    ...
    def __init__(self, var_mode):
        ...
        page_wait()
        sleep(1)
        self.browser.find_element_by_xpath("//*[@id='menu']//*[text()='{0}']".format(self.var_mode)).click()
    ...
    def add(self, var_name, var_type, var_desc, algorithm_list, time_set, list_content, agg_func_set, func_set):
        ...
        # 点击提交
        self.browser.find_element_by_xpath("//*[@id='vareditor-submit']").click()
        log.info("{0}添加: {1}".format(self.var_mode, var_name))

    def _add_common_var(self, var_name):
        self.browser.find_element_by_xpath(
            "//*[contains(@id,'common_var_') and text()='{0}']".format(var_name)).click()
        sleep(1)
    ...
    def _add_advance_var(self, var_name, var_type, var_desc, algorithm_list, time_set, list_content, agg_func_set, func_set):
        ...
        # 变量名称
        if var_name:
            self.browser.find_element_by_xpath("//*[@id='varName']/following-sibling::span//input[1]").clear()
            self.browser.find_element_by_xpath(
                "//*[@id='varName']/following-sibling::span//input[1]").send_keys(var_name)
        ...
        if var_type:
            ...
            self.browser.find_element_by_xpath("//*[contains(@id,'varType') and text()='{0}']".format(var_type)).click()
            log.info("设置变量类型: {0}".format(var_type))
            sleep(1)

        # 变量描述
        if var_desc:
            self.browser.find_element_by_xpath("//*[@id='varDesc']/following-sibling::span//input[1]").clear()
            self.browser.find_element_by_xpath(
                "//*[@id='varDesc']/following-sibling::span//input[1]").send_keys(var_desc)
    ...
    def _kw_opt(self, algorithm_list):
        ...
        if algorithm_list:
            ...
            for algorithm in algorithm_list:
                ...
                if orBracketL:
                    self.browser.find_element_by_xpath(
                        row_xpath + "//*[contains(@class,'orBracketL')]/following-sibling::span[1]//a").click()
                    bracket_elements = self.browser.find_elements_by_xpath(
                        "//*[contains(@id,'combobox') and text()='{0}']".format(orBracketL))
                ...
                if orDataL:
                    self.browser.find_element_by_xpath(
                        row_xpath + "//*[contains(@class,'orDataL')]/following-sibling::span[1]//input[1]").clear()
                    self.browser.find_element_by_xpath(
                        row_xpath + "//*[contains(@class,'orDataL')]/following-sibling::span[1]//input[1]").send_keys(
                        orDataL)
                ...
                if orOperate:
                    self.browser.find_element_by_xpath(
                        row_xpath + "//*[contains(@class,'orOperate')]/following-sibling::span[1]//a").click()
                    operate_elements = self.browser.find_elements_by_xpath(
                        "//*[contains(@id,'combobox') and text()='{0}']".format(orOperate))
                ...
                if orDataR:
                    self.browser.find_element_by_xpath(
                        row_xpath + "//*[contains(@class,'orDataR')]/following-sibling::span[1]//input[1]").clear()
                    self.browser.find_element_by_xpath(
                        row_xpath + "//*[contains(@class,'orDataR')]/following-sibling::span[1]//input[1]").send_keys(
                        orDataR)
                ...
                if orBracketR:
                    self.browser.find_element_by_xpath(
                        row_xpath + "//*[contains(@class,'orBracketR')]/following-sibling::span[1]//a").click()
                    bracket_elements = self.browser.find_elements_by_xpath(
                        "//*[contains(@id,'combobox') and text()='{0}']".format(orBracketR))
                    if len(bracket_elements) == 0:
                        raise NoSuchElementException
                    for element in bracket_elements:
                        if element.is_displayed():
                ...
                if row_num < len(algorithm_list):
                    self.browser.find_element_by_xpath("//*[@id='cols']//a[contains(@class,'add')]").click()
                    row_num += 1
                    sleep(1)

            # 获取预览结果
            preview_element = self.browser.find_element_by_xpath(
                "//*[@id='orPreview']/following-sibling::span/input[2]")
    ...
    def _time_type(self, time_format, custom_time, interval, unit):
        ...
        # 时间格式
        if time_format:
            self.browser.find_element_by_xpath("//*[@id='timeFormatSel']/following-sibling::span[1]//a").click()
            self.browser.find_element_by_xpath(
                "//*[contains(@id,'timeFormatSel') and text()='{0}']".format(time_format)).click()
            log.info("设置时间格式: {0}".format(time_format))

        # 自定义
        if custom_time:
            self.browser.find_element_by_xpath("//*[@id='customTime']/following-sibling::span[1]//input[1]").clear()
            self.browser.find_element_by_xpath(
                "//*[@id='customTime']/following-sibling::span[1]//input[1]").send_keys(custom_time)
            log.info("设置自定义: {0}".format(custom_time))

        # 间隔
        if interval:
            self.browser.find_element_by_xpath("//*[@id='timeInterval']/following-sibling::span[1]//input[1]").clear()
            self.browser.find_element_by_xpath(
                "//*[@id='timeInterval']/following-sibling::span[1]//input[1]").send_keys(interval)
            log.info("设置间隔: {0}".format(interval))

        # 单位
        if unit:
            self.browser.find_element_by_xpath("//*[@id='timeUnit']/following-sibling::span[1]//a").click()
            self.browser.find_element_by_xpath(
                "//*[contains(@id,'timeUnit') and text()='{0}']".format(unit)).click()
    ...
    def _custom_list(self, list_content):
        ...
        if list_content:
            self.browser.find_element_by_xpath(
                "//*[@id='customListValue']/following-sibling::span[1]//textarea").clear()
            self.browser.find_element_by_xpath(
                "//*[@id='customListValue']/following-sibling::span[1]//textarea").send_keys(list_content)
    ...
    def _agg_func(self, col_name, func_name):
        ...
        if col_name:
            self.browser.find_element_by_xpath(
                "//*[@id='aggregateFuncOperateCol']/following-sibling::span[1]//a").click()
            self.browser.find_element_by_xpath(
                "//*[contains(@id,'aggregateFuncOperateCol') and text()='{0}']".format(col_name)).click()
            log.info("设置操作内容: {0}".format(col_name))

        # 函数名称
        if func_name:
            self.browser.find_element_by_xpath(
                "//*[@id='aggregateFuncName']/following-sibling::span[1]//a").click()
            self.browser.find_element_by_xpath(
                "//*[contains(@id,'aggregateFuncName') and text()='{0}']".format(func_name)).click()
    ...
    def _function(self, func_name, col_name, match_type, match_value, match_regex, replace_value):
        ...
        if func_name:
            self.browser.find_element_by_xpath(
                "//*[@id='handleFuncSelect']/following-sibling::span[1]//a").click()
            self.browser.find_element_by_xpath(
                "//*[contains(@id,'handleFuncSelect') and text()='{0}']".format(func_name)).click()
            log.info("设置函数名称: {0}".format(func_name))

        # 操作内容
        if col_name:
            self.browser.find_element_by_xpath(
                "//*[@id='handleFuncCol']/following-sibling::span[1]//a").click()
            self.browser.find_element_by_xpath(
                "//*[contains(@id,'handleFuncCol') and text()='{0}']".format(col_name)).click()
            log.info("设置操作内容: {0}".format(col_name))

        # 匹配方式
        if match_type:
            self.browser.find_element_by_xpath(
                "//*[@name='replaceType']/following-sibling::span[text()='{0}']".format(match_type)).click()
            log.info("设置匹配方式: {0}".format(match_type))
            sleep(1)

        # 匹配值
        if match_value:
            self.browser.find_element_by_xpath("//*[@id='matchValue']/following-sibling::span/input[1]").clear()
            self.browser.find_element_by_xpath(
                "//*[@id='matchValue']/following-sibling::span/input[1]").send_keys(match_value)
            log.info("设置匹配值: {0}".format(match_value))

        # 匹配正则
        if match_regex:
            # 页面划到正则魔方
            regex_cube = self.browser.find_element_by_xpath("//*[@id='regexpReplaceDiv']//h3[text()='正则魔方']")
        ...
        # 替换值
        if replace_value:
            self.browser.find_element_by_xpath("//*[@id='replaceValue']/following-sibling::span/input[1]").clear()
            self.browser.find_element_by_xpath(
                "//*[@id='replaceValue']/following-sibling::span/input[1]").send_keys(replace_value)
