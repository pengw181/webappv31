...
class RegexpTemplate:
    ...
    def delete(self, regexp_name):
        ...
        if alert.title_contains(regexp_name, auto_click_ok=False):
            ...
            if alert.title_contains("操作成功"):
                ...
                log.warn("{0} 删除失败，失败提示: {1}".format(regexp_name, msg))
        else:
            # 无权操作
            log.warn("{0} 删除失败，失败提示: {1}".format(regexp_name, msg))
        set_global_var("ResultMsg", msg, False)
    ...
    def data_clear(self, regexp_name, fuzzy_match=False):
        ...
        self.browser.find_element_by_xpath("//*[@id='keyword']/following-sibling::span/input[1]").send_keys(regexp_name)
        self.browser.find_element_by_xpath("//*[text()='查询']").click()
        page_wait()
        sleep(1)
        if fuzzy_match:
            record_element = self.browser.find_elements_by_xpath(
                "//*[@field='regxTemplName']//*[starts-with(@data-mtips,'{}')]".format(regexp_name))
        else:
            record_element = self.browser.find_elements_by_xpath(
                "//*[@field='regxTemplName']//*[@data-mtips='{0}']".format(regexp_name))
