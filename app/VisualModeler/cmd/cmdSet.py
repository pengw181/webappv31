...
class CmdSet:
    ...
    def search(self, cmd_name=None, cmd_use=None, alive=None, public_cmd=None, cmd_from=None, check_tag=None,
        ...
        if cmd_use:
            ...
            self.browser.find_element_by_xpath("//*[contains(@id,'cmdUse') and text()='{0}']".format(cmd_use)).click()
            log.info("设置指令用途: {0}".format(cmd_use))

        # 启用状态
        if alive:
            self.browser.find_element_by_xpath("//*[@id='isAlive']/following-sibling::span//a").click()
            self.browser.find_element_by_xpath("//*[contains(@id,'isAlive') and text()='{0}']".format(alive)).click()
            log.info("设置启用状态: {0}".format(alive))

        # 公有指令
        if public_cmd:
            self.browser.find_element_by_xpath("//*[@id='isPublicCmd']/following-sibling::span//a").click()
            self.browser.find_element_by_xpath(
                "//*[contains(@id,'isPublicCmd') and text()='{0}']".format(public_cmd)).click()
            log.info("设置公有指令: {0}".format(public_cmd))

        # 指令来源
        if cmd_from:
            self.browser.find_element_by_xpath("//*[@id='isDown']/following-sibling::span//a").click()
            self.browser.find_element_by_xpath("//*[contains(@id,'isDown') and text()='{0}']".format(cmd_from)).click()
            log.info("设置指令来源: {0}".format(cmd_from))

        # 审批状态
        if check_tag:
            self.browser.find_element_by_xpath("//*[@id='checkTag']/following-sibling::span//a").click()
            self.browser.find_element_by_xpath(
                "//*[contains(@id,'checkTag') and text()='{0}']".format(check_tag)).click()
        ...
        # 点击查询
        self.browser.find_element_by_xpath("//*[@id='cmdSet-query']").click()
        page_wait()
        ...
        try:
            self.browser.find_element_by_xpath(
                "//*[@class='datagrid-body']//*[contains(@id,'cmdInfoTab')]//*[@class='datagrid-cell-rownumber' and text()='1']")
            log.info("查询有结果")
            return True
    ...
    def cmd_page(self, cmd_name, cmd_category, cmd_use,  public_cmd, sensitive_cmd, cmd_timeout, command, remark,
        ...
        # 提交
        self.browser.find_element_by_xpath("//*[@id='cmdSet-submit']").click()
        alert = BeAlertBox()
        if alert.exist_alert:
            msg = alert.get_msg()
            if alert.title_contains("保存成功"):
                ...
                log.warn("指令集保存失败，失败提示: {0}".format(msg))
        else:
            log.warn("没有弹出框提示信息")
            msg = None
        set_global_var("ResultMsg", msg, False)
    ...
    def delete(self, cmd_info):
        ...
        if alert.title_contains(cmd_name, auto_click_ok=False):
            ...
            if alert.title_contains("操作成功"):
                ...
                log.warn("{0} 删除失败，失败提示: {1}".format(cmd_name, msg))
        else:
            # 无权操作
            log.warn("{0} 删除失败，失败提示: {1}".format(cmd_name, msg))
        set_global_var("ResultMsg", msg, False)
    ...
    def update_status(self, cmd_info, status):
        ...
        model = cmd_info.get("设备型号")
        self.choose(cmd_name, level, vendor, model)
        cmd = self.browser.find_element_by_xpath(
            "//*[contains(@id,'cmdInfoTab')]//*[text()='{0}']/../../..".format(cmd_name))
        ...
        if tmp ^ current_status:
            self.browser.find_element_by_xpath(
                "//*[contains(@id,'cmdInfoTab')]//*[text()='{0}']/../../following-sibling::td[4]//*[@class='switchbutton']".format(
                    cmd_name)).click()
            ...
            if alert.title_contains("启用成功"):
                ...
                log.warn("{0}指令失败，失败原因: {1}".format(status, msg))
            set_global_var("ResultMsg", msg, False)
        else:
    ...
    def data_clear(self, cmd_name, fuzzy_match=False):
        ...
        self.browser.find_element_by_xpath("//*[@id='cmdKeyword']/following-sibling::span/input[1]").send_keys(cmd_name)
        ...
        if fuzzy_match:
            record_element = self.browser.find_elements_by_xpath(
                "//*[@field='cmdName']//*[starts-with(@data-mtips,'{}')]".format(cmd_name))
        else:
            record_element = self.browser.find_elements_by_xpath(
                "//*[@field='cmdName']//*[@data-mtips='{0}']".format(cmd_name))
