...
class JoinModeEData:
    ...
    def add_join_table(self, join_table_list, join_type, left_table_set, right_table_set, new_table_name, field,
        ...
        """
        log.info("开始添加join模式")
        self.browser.find_element_by_xpath(self.tab_xpath + "//*[@id='edata-addJoin']").click()
        # 进入表配置页面iframe
        ...
        if join_table_list:
            for table_name in join_table_list:
                self.browser.find_element_by_xpath(
                    "//*[@id='tableSearch']/following-sibling::span/input[1]").clear()
                self.browser.find_element_by_xpath(
                    "//*[@id='tableSearch']/following-sibling::span/input[1]").send_keys(table_name)
                self.browser.find_element_by_xpath(
                    "//*[@id='table_info']//span[text()='{0}']".format(table_name)).click()
                log.info("选择合并表: {0}".format(table_name))
                sleep(1)

        # 划到第二步
        step_element = self.browser.find_element_by_xpath("//*[text()='第二步: 配置表间的关联字段，选择查询结果显示字段']")
        ...
        if join_type:
            if join_type == "左关联":
                self.browser.find_element_by_xpath("//*[@id='leftJoin']").click()
            else:
                self.browser.find_element_by_xpath("//*[@id='rightJoin']").click()
        ...
        if left_table_set:
            for col_set in left_table_set:
                ...
                if col_set.__contains__("合并后显示"):
                    ...
                    if show_col == "是":
                        self.browser.find_element_by_xpath(
                            "//*[@id='leftTable']//*[text()='{0}']/../following-sibling::td[2]".format(
                                col_name)).click()
                ...
                if col_set.__contains__("关联列"):
                    ...
                    self.browser.find_element_by_xpath(
                        "//*[@id='leftTable']//*[text()='{0}']/../following-sibling::td[3]//a".format(
                            col_name)).click()
                    on_elements = self.browser.find_elements_by_xpath(
                        "//*[contains(@id,'combobox') and text()='{0}']".format(add_on))
        ...
        if right_table_set:
            for col_set in right_table_set:
                ...
                if col_set.__contains__("合并后显示"):
                    ...
                    if show_col == "是":
                        self.browser.find_element_by_xpath(
                            "//*[@id='rightTable']//*[text()='{0}']/../following-sibling::td[2]".format(
                                col_name)).click()
                ...
                if col_set.__contains__("关联列"):
                    add_on = col_set.get("关联列")
                    self.browser.find_element_by_xpath(
                        "//*[@id='rightTable']//*[text()='{0}']/../following-sibling::td[3]//a".format(
                            col_name)).click()
                    on_elements = self.browser.find_elements_by_xpath(
                        "//*[contains(@id,'combobox') and text()='{0}']".format(add_on))
                    if len(on_elements) > 0:
        ...
        # 点击确定已选关系
        self.browser.find_element_by_xpath("//*[@id='join-confirmRel']").click()
        # 划到第三步
        step_element = self.browser.find_element_by_xpath("//*[text()='第三步: 合并后，新表配置']")
        self.browser.execute_script("arguments[0].scrollIntoView(true);", step_element)
        sleep(1)

        # 数据表名称
        if new_table_name:
            self.browser.find_element_by_xpath("//*[@id='tableNameCh']/following-sibling::span/input[1]").clear()
            self.browser.find_element_by_xpath(
                "//*[@id='tableNameCh']/following-sibling::span/input[1]").send_keys(new_table_name)
            log.info("设置数据表名称: {0}".format(new_table_name))

        # 专业领域
        if field:
            self.browser.find_element_by_xpath("//*[@id='tempTypeId']/following-sibling::span//a").click()
            sleep(1)
            ...
            for col_set in new_table_set:
                ...
                if col_set.__contains__("新列名"):
                    ...
                    if new_col_name != old_col_name:
                        # 修改新列名
                        new_col_ele = self.browser.find_element_by_xpath(row_obj_xpath + "/following-sibling::td[1]")
                        new_col_ele.click()
                        self.browser.find_element_by_xpath("//*[@id='popup_box']/div/div/span/input[1]").clear()
                        self.browser.find_element_by_xpath(
                            "//*[@id='popup_box']/div/div/span/input[1]").send_keys(new_col_name)
                        self.browser.find_element_by_xpath("//*[@onclick='confirmBox($(this))']").click()
                        ...
                        self.browser.find_element_by_xpath(row_obj_xpath + "/following-sibling::td[2]").click()
        ...
        # 保存新表
        self.browser.find_element_by_xpath("//*[@id='join-save']").click()
        ...
        if alert.title_contains("保存成功"):
            ...
            log.warn("{0} 添加失败，失败提示: {1}".format(new_table_name, msg))
    ...
    def add_union_table(self, join_table_list, join_type, join_table_set, new_table_name, field, new_table_set):
        ...
        self.browser.find_element_by_xpath(self.tab_xpath + "//*[@id='edata-addUnion']").click()
        # 进入表配置页面iframe
        ...
        # 划到第二步
        step_element = self.browser.find_element_by_xpath("//*[text()='第二步: 选择各表需进行合并的字段并排序']")
        self.browser.execute_script("arguments[0].scrollIntoView(true);", step_element)
        sleep(1)

        # 关联方式
        if join_type:
            self.browser.find_element_by_xpath("//*[@name='joinType' and @value='{0}']".format(join_type)).click()
            log.info("设置关联方式: {0}".format(join_type))
            ...
            for col_set in join_table_set:
                ...
                if col_set.__contains__("合并列"):
                    ...
                    for col in union_col_list:
                        self.browser.find_element_by_xpath(
                            "//*[text()='{0}']/../following-sibling::div[1]//td[text()='{1}']".format(
            ...
            for col_set in new_table_set:
                ...
                if col_set.__contains__("新列名"):
                    ...
                    if new_col_name != old_col_name:
                        ...
                        # 修改新列名，会自动勾选"搜索条件"，先勾掉
                        self.browser.find_element_by_xpath(row_obj_xpath + "/following-sibling::td[4]").click()
