...
class TableModeEData:
    ...
    def configure_update_rule(self, cmd, rulerX, result_bind):
        ...
        self.browser.find_element_by_xpath("//*[@id='showPng']").click()
        sleep(1)
        steps_row_element = self.browser.find_element_by_xpath("//*[@class='steps_row'][1]")
        ...
        # 指令解析模版
        if rulerX:
            self.browser.find_element_by_xpath("//*[@id='rulerxRow']/following-sibling::span//a").click()
            # 进入绑定指令解析模版页面iframe
            wait = WebDriverWait(self.browser, 30)
            wait.until(ec.frame_to_be_available_and_switch_to_it(
            ...
            page_wait()
            # 点击解析模版
            self.browser.find_element_by_xpath("//*[@field='analyzerName']//*[text()='{}']".format(rulerX)).click()
            # 确定
        ...
        # 二维表结果绑定
        if result_bind:
            bind_row_element = self.browser.find_element_by_xpath("//*[text()='二维表结果绑定:']")
            self.browser.execute_script("arguments[0].scrollIntoView(true);", bind_row_element)
            i = 1
            for col_name in result_bind:
                self.browser.find_element_by_xpath("//*[@id='edataFormatTable']//th[{0}]//a".format(i)).click()
        ...
        # 保存
        self.browser.find_element_by_xpath("//*[@id='tablemode-updateRuleCfgSave']").click()
        ...
        if alert.title_contains("保存成功", auto_click_ok=False):
            ...
            log.warn("配置更新规则失败，失败提示: {0}".format(msg))
