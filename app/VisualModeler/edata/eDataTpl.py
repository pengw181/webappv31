...
class EDataTemplate:
    ...
    def __init__(self, temp_type):
        ...
        # 点击模式tab
        self.browser.find_element_by_xpath("//*[text()='{0}']".format(temp_type)).click()
        page_wait(5)
    ...
    def choose(self, table_name):
        ...
        self.browser.find_element_by_xpath(
            self.tab_xpath + "//*[@id='tableName']/following-sibling::span/input[1]").clear()
        self.browser.find_element_by_xpath(
            self.tab_xpath + "//*[@id='tableName']/following-sibling::span/input[1]").send_keys(table_name)
        # 点击查询
        self.browser.find_element_by_xpath(self.tab_xpath + "//*[@id='edata-query']").click()
        page_wait()
        templates = self.browser.find_elements_by_xpath(
            "//*[contains(@id,'edataTmplTab')]/*[@field='tableNameCh']//*[text()='{0}']".format(table_name))
        if len(templates) > 0:
    ...
    def add_table(self, table_name, field, remark, cmd, regexp_start, regexp_end, sample):
        ...
        """
        log.info("开始添加数据")
        self.browser.find_element_by_xpath(self.tab_xpath + "//*[@id='edata-add']").click()
        # 进入表配置页面iframe
        wait = WebDriverWait(self.browser, 30)
        ...
        # 保存
        self.browser.find_element_by_xpath("//*[@id='tableNameCfg-save']").click()
        ...
        if alert.title_contains("保存成功"):
            ...
            log.warn("{0} 添加失败，失败提示: {1}".format(table_name, msg))
    ...
    def update_table(self, obj, table_name, field, remark, cmd, regexp_start, regexp_end, sample):
        ...
        log.info("开始修改数据")
        self.choose(obj)
        self.browser.find_element_by_xpath(self.tab_xpath + "//*[@id='edata-update']").click()
        ...
        if alert.exist_alert:
            ...
            wait = WebDriverWait(self.browser, 30)
            wait.until(ec.frame_to_be_available_and_switch_to_it((By.XPATH, self.table_main_iframe_xpath)))
            self.browser.find_element_by_xpath("//*[@id='cfg-tableName']").click()
            # 进入表名配置页面iframe
            wait = WebDriverWait(self.browser, 30)
            wait.until(ec.frame_to_be_available_and_switch_to_it((By.XPATH, self.table_name_iframe_xpath)))
            sleep(1)
    ...
    def col_sets_page(self, col_set):
        ...
        for col in col_set:
            ...
            if opt_type:
                if opt_type == "添加":
                    self.browser.find_element_by_xpath("//*[@id='cfg-col']").click()
                elif opt_type == "修改":
                    self.browser.find_element_by_xpath(
    ...
    def bind_netunit(self, table_name, netunit_list):
        ...
        if self.temp_type in ["二维表模式", "列更新模式", "分段模式"]:
            ...
            # 点击绑定网元
            self.browser.find_element_by_xpath(self.tab_xpath + "//*[@id='edata-bindNE']").click()
            # 判断是否进入绑定网元页面
            ...
            assigned_ne = self.browser.find_elements_by_xpath("//*[contains(@id,'assignedNetunitTab')]//*[text()='1']")
            if len(assigned_ne) > 0:
                sleep(1)
                log.info("存在已分配网元，先移除")
                # 移除右侧所有网元
                self.browser.find_element_by_xpath("//*[@class='operatorBtn']/button[4]").click()
                self.browser.switch_to.default_content()
                # 进入模版配置列表页面
                self.browser.switch_to.frame(self.browser.find_element_by_xpath(self.eData_iframe_xpath))
                alert = BeAlertBox(back_iframe=False)
            ...
            for ne in netunit_list:
                ...
                if ne.__contains__("网元名称"):
                    ne_name = ne.get("网元名称")
                    self.browser.find_element_by_xpath(
                        "//*[@id='bindingNetunitName']/following-sibling::span/input[1]").clear()
                    self.browser.find_element_by_xpath(
                        "//*[@id='bindingNetunitName']/following-sibling::span/input[1]").send_keys(ne_name)
                ...
                if ne.__contains__("设备型号"):
                    model = ne.get("设备型号")
                    self.browser.find_element_by_xpath(
                        "//*[@id='bindingNetunitModelNE']/following-sibling::span//a").click()
                    self.browser.find_element_by_xpath(
                        "//*[contains(@id,'bindingNetunitModelNE') and text()='{0}']".format(model)).click()
                    log.info("设置设备型号: {0}".format(model))

                # 点击查询待分配
                self.browser.find_element_by_xpath("//*[text()='查询待分配']").click()
                page_wait()

                unassigned_ne = self.browser.find_elements_by_xpath(
                    "//*[contains(@id,'unassignedNetunitTab')]//*[text()='1']")
                if len(unassigned_ne) == 0:
                    raise AttributeError("网元列表里的数据格式需要是字典")

                # 分配左侧所有网元
                self.browser.find_element_by_xpath("//*[@class='operatorBtn']/button[1]").click()
        ...
        if set_status == "启用":
            if current_status:
                ...
                if self.temp_type == "合并模式":
                    self.browser.find_element_by_xpath(
                        "//*[text()='{0}']/../../following-sibling::td[4]/div/span".format(table_name)).click()
                else:
                    self.browser.find_element_by_xpath(
                        "//*[text()='{0}']/../../following-sibling::td[3]/div/span".format(table_name)).click()
                ...
                if alert.title_contains("启用模版成功", auto_click_ok=False):
                    ...
                    log.warn("{0} 启用失败，失败提示: {1}".format(table_name, msg))
                    set_global_var("ResultMsg", msg, False)
        elif set_status == "禁用":
            if current_status:
                if self.temp_type == "合并模式":
                ...
                if alert.title_contains("您确定禁用所选模版吗", auto_click_ok=False):
                    ...
                    if alert.title_contains("禁用成功"):
                        ...
                        log.warn("{0} 禁用失败，失败提示: {1}".format(table_name, msg))
                else:
                    log.warn("{0} 禁用失败，失败提示: {1}".format(table_name, msg))
                set_global_var("ResultMsg", msg, False)
            else:
    ...
    def delete_table(self, obj):
        ...
        log.info("开始删除数据")
        self.choose(obj)
        self.browser.find_element_by_xpath(self.tab_xpath + "//*[@id='edata-del']").click()
    ...
    def data_clear(self, obj, fuzzy_match=False):
        ...
        self.browser.find_element_by_xpath(
            self.tab_xpath + "//*[@id='tableName']/following-sibling::span/input[1]").clear()
        self.browser.find_element_by_xpath(
            self.tab_xpath + "//*[@id='tableName']/following-sibling::span/input[1]").send_keys(obj)
        # 点击查询
        self.browser.find_element_by_xpath(self.tab_xpath + "//*[@id='edata-query']//*[text()='查询']").click()
        page_wait()
        if fuzzy_match:
            record_element = self.browser.find_elements_by_xpath(
                "//*[contains(@id,'edataTmplTab')]/*[@field='tableNameCh']//*[starts-with(text(),'{0}')]".format(obj))
        else:
            record_element = self.browser.find_elements_by_xpath(
                "//*[contains(@id,'edataTmplTab')]/*[@field='tableNameCh']//*[text()='{0}']".format(obj))
                ...
                if alert.title_contains("您确定需要删除{0}吗".format(search_result), auto_click_ok=False):
                    ...
                    if alert.title_contains("删除成功"):
                        ...
                        if fuzzy_match:
                            # 重新获取页面查询结果
                            record_element = self.browser.find_elements_by_xpath(
                                "//*[contains(@id,'edataTmplTab')]/*[@field='tableNameCh']//*[starts-with(text(),'{0}')]".format(
                                    obj))
                            if len(record_element) > 0:
                                exist_data = True
                            else:
