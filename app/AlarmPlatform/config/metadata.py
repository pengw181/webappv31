...
class MetaData:
    ...
    def choose(self, metadata_name):
        ...
        """
        input_ele = self.browser.find_element_by_xpath("//*[@id='metadataName']/following-sibling::span[1]/input[1]")
        input_ele.clear()
        input_ele.send_keys(metadata_name)
        self.browser.find_element_by_xpath("//span[text()='查询']").click()
        page_wait()
        self.browser.find_element_by_xpath("//*[@field='metadataName']//a[text()='{}']".format(metadata_name)).click()
    ...
    def add(self, metadata_name, database, table_belong, data_delay, remark, time_field, time_field_format,
        ...
        if alert.title_contains("提交成功"):
            ...
            log.warn("数据 {0} 添加失败，失败提示: {1}".format(metadata_name, msg))
    ...
    def metadata_page(self, metadata_name, database, table_belong, data_delay, remark, time_field, time_field_format,
        ...
        if metadata_name:
            self.browser.find_element_by_xpath(
                "//*[@id='editDiv']//*[@id='metadataName']/following-sibling::span/input[1]").clear()
            self.browser.find_element_by_xpath(
                "//*[@id='editDiv']//*[@id='metadataName']/following-sibling::span/input[1]").send_keys(metadata_name)
            log.info("设置元数据名称: {0}".format(metadata_name))

        # 数据库
        if database:
            self.browser.find_element_by_xpath(
                "//*[@id='editDiv']//*[@id='databaseInfoId']/following-sibling::span/input[1]").click()
            sleep(1)
            self.browser.find_element_by_xpath(
                "//*[contains(@id,'databaseInfoId') and text()='{}']".format(database)).click()
            log.info("设置数据库: {0}".format(database))

        # 表中文名
        if table_belong:
            self.browser.find_element_by_xpath(
                "//*[@id='editDiv']//*[@id='tableBelongId']/following-sibling::span/input[1]").click()
            sleep(1)
            self.browser.find_element_by_xpath(
                "//*[contains(@id,'tableBelongId') and text()='{}']".format(table_belong)).click()
        ...
        data_col_obj = self.browser.find_element_by_xpath(
            "//*[contains(@id,'prepareFieldAreaDiv') and @datagrid-row-index='0']/*[@field='COLUMN_NAME']/div")
        ...
        if data_delay:
            self.browser.find_element_by_xpath(
                "//*[@id='editDiv']//*[@id='dataDelay']/following-sibling::span/input[1]").clear()
            self.browser.find_element_by_xpath(
                "//*[@id='editDiv']//*[@id='dataDelay']/following-sibling::span/input[1]").send_keys(data_delay)
            log.info("设置数据时延: {0}".format(data_delay))

        # 备注
        if remark:
            remark_textarea = self.browser.find_element_by_xpath("//*[@id='remark']")
        ...
        if time_field:
            self.browser.find_element_by_xpath(
                "//*[@id='editDiv']//*[@id='timeField']/following-sibling::span/input[1]").click()
            sleep(1)
            # 字段自动转换
            ...
            self.browser.find_element_by_xpath(
                "//*[contains(@id,'timeField') and text()='{}']".format(time_field)).click()
            log.info("设置时间字段: {0}".format(time_field))

        # 时间格式
        if time_field_format:
            self.browser.find_element_by_xpath(
                "//*[@id='editDiv']//*[@id='timeFieldType']/following-sibling::span/input[1]").click()
            sleep(1)
            self.browser.find_element_by_xpath(
                "//*[contains(@id,'timeFieldType') and text()='{}']".format(time_field_format)).click()
            log.info("设置时间格式: {0}".format(time_field_format))

        # 待选字段
        if prepare_field:
            # 先清空已被选择字段
            selected_ele = self.browser.find_elements_by_xpath(
                "//*[@id='prepareFieldAreaDivRow']/div/div[2]/div/div[2]//*[contains(@id,'prepareFieldAreaDiv') and contains(@class,'selected')]")
            ...
            for p in prepare_field:
                ...
                if self.upperOrLower == "upper":
                    ...
                    p = p.lower()
                to_selected_field = self.browser.find_element_by_xpath(
                    "//*[contains(@class,'COLUMN_NAME') and text()='{0}']".format(p))
            ...
            if isinstance(selected_field, dict):
                for key, value in selected_field.items():
                    ...
                    if self.upperOrLower == "upper":
                        ...
                        key = key.lower()
                    self.browser.find_element_by_xpath(
                        "//*[contains(@class,'colNameEn') and text()='{0}']/../following-sibling::td[4]//*[text()='编辑']".format(
                            key)).click()
                    ...
                    if b_name:
                        self.browser.find_element_by_xpath(
                            "//*[contains(@class,'colNameEn') and text()='{0}']/../following-sibling::td[1]//input".format(
                                key)).clear()
                        self.browser.find_element_by_xpath(
                            "//*[contains(@class,'colNameEn') and text()='{0}']/../following-sibling::td[1]//input".format(
                    ...
                    if d_name:
                        self.browser.find_element_by_xpath(
                            "//*[contains(@class,'colNameEn') and text()='{0}']/../following-sibling::td[2]//input".format(
                                key)).clear()
                        self.browser.find_element_by_xpath(
                            "//*[contains(@class,'colNameEn') and text()='{0}']/../following-sibling::td[2]//input".format(
                                key)).send_keys(d_name)
                    self.browser.find_element_by_xpath(
                        "//*[contains(@class,'colNameEn') and text()='{0}']/../following-sibling::td[4]//*[text()='确定']".format(
                            key)).click()
        ...
        # 保存
        self.browser.find_element_by_xpath("//*[@id='saveButtonId']//*[text()='保存']").click()
    ...
    def data_clear(self, obj, fuzzy_match=False):
        ...
        self.browser.find_element_by_xpath("//*[@id='metadataName']/following-sibling::span[1]/input[1]").clear()
        self.browser.find_element_by_xpath(
            "//*[@id='metadataName']/following-sibling::span[1]/input[1]").send_keys(obj)
        self.browser.find_element_by_xpath("//*[@id='btn']//*[text()='查询']").click()
        page_wait()
        if fuzzy_match:
            record_element = self.browser.find_elements_by_xpath(
                "//*[@field='metadataName']/*[contains(@class,'metadataName')]/*[starts-with(text(),'{}')]".format(obj))
        else:
            record_element = self.browser.find_elements_by_xpath(
                "//*[@field='metadataName']/*[contains(@class,'metadataName')]/*[text()='{}']".format(obj))
                ...
                if alert.title_contains("您确定需要删除{0}吗".format(search_result), auto_click_ok=False):
                    ...
                    if alert.title_contains("成功"):
                        ...
                        if fuzzy_match:
                            # 重新获取页面查询结果
                            record_element = self.browser.find_elements_by_xpath(
                                "//*[@field='metadataName']/*[contains(@class,'metadataName')]/*[starts-with(text(),'{0}')]".format(
                                    obj))
                            if len(record_element) > 0:
                                exist_data = True
                            else:
