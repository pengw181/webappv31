...
from common.page.func.tableData import get_table_data2
...
from common.page.func.pagination import Pagination
from common.date.dateUtil import set_calendar, set_laydate
...
class AlarmRule:
    ...
    def choose(self, rule_name):
        ...
        """
        input_ele = self.browser.find_element_by_xpath("//*[@name='alarmRuleName']/preceding-sibling::input")
        input_ele.clear()
        input_ele.send_keys(rule_name)
        self.browser.find_element_by_xpath("//span[text()='查询']").click()
        page_wait()
        self.browser.find_element_by_xpath("//*[@field='alarmRuleName']//*[text()='{}']".format(rule_name)).click()
    ...
    def add(self, alarm_type, alarm_plan, basic_conf, dimension_conf, filter_conf, result_conf, storage_conf):
        ...
        if alarm_type:
            self.browser.find_element_by_xpath(
                "//*[@class='alarm-edit-form']//*[@comboname='alarmTypeId']/following-sibling::span[1]//a").click()
            self.browser.find_element_by_xpath(
        ...
        if alarm_plan:
            self.browser.find_element_by_xpath(
                "//*[@class='alarm-edit-form']//*[@comboname='alarmPlanId']/following-sibling::span[1]//a").click()
            sleep(1)
            self.browser.find_element_by_xpath("//*[contains(@id,'alarmPlanId') and text()='{0}']".format(
                alarm_plan)).click()
            log.info("设置告警计划: {0}".format(alarm_plan))

        # 点击确定进入基本信息配置
        self.browser.find_element_by_xpath("//*[@class='form_row']/following-sibling::div//*[text()='确定']").click()
        self.browser.switch_to.parent_frame()
        wait = WebDriverWait(self.browser, 10)
        wait.until(ec.frame_to_be_available_and_switch_to_it((
        ...
        if alert.title_contains("保存成功"):
            ...
            log.warn("数据 {0} 添加失败，失败提示: {1}".format(self.rule_name, msg))
    ...
    def rule_page(self, basic_conf, dimension_conf, filter_conf, result_conf, storage_conf):
        ...
        self.browser.find_element_by_xpath("//*[@id='rule_name_next']").click()
        wait = WebDriverWait(self.browser, 10)
        ...
        self.browser.find_element_by_xpath("//*[@id='rule_dimension_next']").click()
        wait = WebDriverWait(self.browser, 10)
        ...
        self.browser.find_element_by_xpath("//*[@id='rule_config_next']").click()
        wait = WebDriverWait(self.browser, 10)
        ...
        self.browser.find_element_by_xpath("//*[@id='rule_result_next']").click()
        wait = WebDriverWait(self.browser, 10)
        ...
        # 告警存储配置页面点击保存，保存整个配置
        self.browser.find_element_by_xpath("//*[@id='rule_result_output_next']").click()
    ...
    def basic_config(self, rule_name, rule_desc, alarm_level, valid_begin_time, valid_end_time, start_time, ym,
        ...
        # 规则名称
        if rule_name:
            self.browser.find_element_by_xpath("//*[@id='add_rule_name']").clear()
            self.browser.find_element_by_xpath("//*[@id='add_rule_name']").send_keys(rule_name)
            log.info("设置规则名称: {0}".format(rule_name))

        # 规则描述
        if rule_desc:
            rule_desc_textarea = self.browser.find_element_by_xpath("//*[@id='add_rule_desc']")
        ...
        # 告警等级
        if alarm_level:
            self.browser.find_element_by_xpath("//*[@id='rule_config_level']//i").click()
            sleep(1)
            alarm_level_ele = self.browser.find_element_by_xpath(
                "//*[@id='rule_config_level']//dd[text()='{0}']".format(alarm_level))
        ...
        # 有效开始时间
        if valid_begin_time:
            self.browser.find_element_by_xpath("//*[@id='rule_task_config_startTime']").click()
            set_laydate(date_s=valid_begin_time, date_format='%Y-%m-%d %H:%M:%S')
            log.info("设置有效开始时间: {0}".format(valid_begin_time))

        # 有效结束时间
        if valid_end_time:
            self.browser.find_element_by_xpath("//*[@id='rule_task_config_endTime']").click()
            set_laydate(date_s=valid_end_time, date_format='%Y-%m-%d %H:%M:%S')
            log.info("设置有效结束时间: {0}".format(valid_end_time))

        # 开始时间
        if start_time:
            self.browser.find_element_by_xpath("//*[@id='rule_config_trigger_time']").click()
            set_laydate(date_s=start_time, date_format='%Y-%m-%d %H:%M:%S')
            log.info("设置开始时间: {0}".format(start_time))

        # 同比/环比
        if ym:
            self.browser.find_element_by_xpath("//*[@id='select_yqoyq_div']//i").click()
            self.browser.find_element_by_xpath("//*[@id='select_yqoyq_div']//dd[text()='{}']".format(ym)).click()
            log.info("设置同比/环比: {0}".format(ym))

        # 连续次数
        if series_time:
            self.browser.find_element_by_xpath("//*[@id='rule_alarm_config_isSeries']").clear()
            self.browser.find_element_by_xpath("//*[@id='rule_alarm_config_isSeries']").send_keys(series_time)
            log.info("设置连续次数: {0}".format(series_time))

        # 检测频率
        if rate:
            self.browser.find_element_by_xpath("//*[@id='rule_alarm_rate']").clear()
            self.browser.find_element_by_xpath("//*[@id='rule_alarm_rate']").send_keys(rate)
            log.info("设置连续次数: {0}".format(rate))

        # 检测频率单位
        if rate_unit:
            self.browser.find_element_by_xpath("//*[@id='rule_config_rateunit']//i").click()
            self.browser.find_element_by_xpath(
                "//*[@id='rule_config_rateunit']//dd[text()='{}']".format(rate_unit)).click()
            log.info("设置检测频率单位: {0}".format(rate_unit))

        # 同/环比字段
        if compare_field:
            self.browser.find_element_by_xpath("//*[@id='yq_field']//i").click()
            sleep(1)
            compare_field_ele = self.browser.find_element_by_xpath(
                "//*[@id='yq_field']//dd[text()='{}']".format(compare_field))
            action = ActionChains(self.browser)
        ...
        # 字段基值
        if yq_base_value:
            self.browser.find_element_by_xpath("//*[@id='yq_base_value']").clear()
            self.browser.find_element_by_xpath("//*[@id='yq_base_value']").send_keys(yq_base_value)
            log.info("设置字段基值: {0}".format(yq_base_value))

        # 周期单位
        if yq_period:
            self.browser.find_element_by_xpath("//*[@id='select_yqoyq_period_div']//i").click()
            self.browser.find_element_by_xpath(
                "//*[@id='select_yqoyq_period_div']//dd[text()='{}']".format(yq_period)).click()
            log.info("设置周期单位: {0}".format(yq_period))

        # 获取当前告警规则名称
        self.rule_name = self.browser.find_element_by_xpath("//*[@id='add_rule_name']").get_attribute("value")
    ...
    def dimension_config(self, domain):
        ...
        """
        # 获取领域勾选状态
        domain_checkbox = self.browser.find_element_by_xpath("//*[text()='领域']/..")
        ...
        if domain:
            if not checked_status:
                self.browser.find_element_by_xpath("//*[text()='领域']").click()
                sleep(1)
            log.info("勾选【领域】")

            for d in domain:
                self.browser.find_element_by_xpath("//*[@title='{0}']/following-sibling::div[1]/span".format(d)).click()
    ...
    def filter_config(self, filter_condition, alarm_area_config, view_config, test_config):
        ...
        if filter_condition:
            # 先点击清空
            self.browser.find_element_by_xpath("//*[@id='delete_all_where_item']").click()
            focus_area = self.browser.find_element_by_xpath("//*[@id='alarm_rule_config_div']//*[text()='公共标签']")
            ...
            for f in filter_condition:
                ...
                # 判断字段标签是大写或小写
                data_col_obj = self.browser.find_element_by_xpath("//*[@ID='field_item']/li[2]")
                ...
                # 标签类型
                if tag_type == "公共标签":
                    tag_element = self.browser.find_element_by_xpath(
                        "//*[@id='common_item']/li[@fieldchinesename='{0}']".format(tag_name))
                else:
                    # 字段标签转换大小写
                    ...
                    tag_element = self.browser.find_element_by_xpath(
                        "//*[@id='field_item']/li[@fieldchinesename='{0}']".format(tag_name))
                filter_trigger = self.browser.find_element_by_xpath("//*[@id='editable_trigger']")
                action = ActionChains(self.browser)
                # 将标签拖到过滤条件框中
                ...
                # 给标签设置属性
                if tag_type == "公共标签":
                    tag_elements = self.browser.find_elements_by_xpath(
                        "//*[@id='editable_trigger']//*[contains(@id,'common_item_li') and @fieldchinesename='{0}']".format(
                            tag_name))
                else:
                    tag_elements = self.browser.find_elements_by_xpath(
                        "//*[@id='editable_trigger']//*[contains(@id,'field_item_li') and @fieldchinesename='{0}']".format(
                            tag_name))
                    ...
                    if tag_attribute:
                        if tag_type == "公共标签":
                            if tag_name == "自定义过滤条件":
                                ...
                                self.browser.find_element_by_xpath(
                                    "//*[@id='where_customize_sql_value']").send_keys(customize_sql)
                        else:
                            ...
                            if logic_operator:
                                self.browser.find_element_by_xpath(
                                    "//*[@id='where_select_logic_operator']/following-sibling::div[1]//i").click()
                                sleep(1)
                                logic_ele = self.browser.find_element_by_xpath(
                                    "//*[@id='where_select_logic_operator']/following-sibling::div[1]//dd[text()='{0}']".format(
                                        logic_operator))
                            ...
                            # 字段类型，默认字符
                            if field_type == "数值":
                                input_text_ele = self.browser.find_element_by_xpath(
                                    "//*[@id='where_item_number_single_value']")
                            else:
                                input_text_ele = self.browser.find_element_by_xpath(
                                    "//*[@id='where_customer_tag_text_value']")
                            # 字段值
                            if tag_text_value:
                        ...
                        # 点击保存
                        self.browser.find_element_by_xpath("//*[@id='save_customer_tag_text']").click()
                else:
                    raise KeyError("标签【{0}】未加入到过滤条件，请检查")

        # 告警区域配置
        if alarm_area_config:
            editable_field = self.browser.find_element_by_xpath("//*[@id='editable_field']")
            self.browser.execute_script("arguments[0].scrollIntoView(true);", editable_field)
            # 先点击清空
            self.browser.find_element_by_xpath("//*[@id='delete_all_result_item']").click()
            for tag, tag_info in alarm_area_config.items():
                ...
                # 将标签拖到过滤条件框中
                tag_element = self.browser.find_element_by_xpath(
                    "//*[@id='alarm_rule_config_div']//*[contains(@id,'field_item') and @fieldchinesename='{0}']".format(
                        tag))
                alarm_area = self.browser.find_element_by_xpath("//*[@id='editable_field']")
                action.drag_and_drop(tag_element, alarm_area).perform()
                log.info("告警区域配置加入标签【{0}】".format(tag))

                # 点击字段标签
                tag_elements = self.browser.find_elements_by_xpath(
                    "//*[@id='result_row_left']//*[contains(@id,'field_item') and @fieldchinesename='{0}']".format(tag))
                if len(tag_elements) > 0:
                    # 选择匹配到的最后一个标签，表示最新加入的标签
                    element = tag_elements[-1]
                    element.click()
                    ...
                    if agg_function:
                        self.browser.find_element_by_xpath(
                            "//*[@id='select_agg_function']/following-sibling::div[1]//i").click()
                        self.browser.find_element_by_xpath(
                            "//*[@id='select_agg_function']/following-sibling::div[1]//dd[text()='{0}']".format(
                                agg_function)).click()
                        log.info("选择聚合函数: {0}".format(agg_function))
                    # 结果名称
                    if agg_result_name:
                        self.browser.find_element_by_xpath("//*[@id='input_agg_result_alias_name']").clear()
                        self.browser.find_element_by_xpath(
                            "//*[@id='input_agg_result_alias_name']").send_keys(agg_result_name)
                        log.info("设置结果名称: {0}".format(agg_result_name))
                    # 阈值条件
                    if agg_logic:
                        self.browser.find_element_by_xpath(
                            "//*[@id='select_logic_operator']/following-sibling::div[1]//i").click()
                        self.browser.find_element_by_xpath(
                            "//*[@id='select_logic_operator']/following-sibling::div[1]//dd[text()='{0}']".format(
                                agg_logic)).click()
                        log.info("设置阈值条件: {0}".format(agg_logic))
                    # 告警阈值
                    if agg_threshold:
                        self.browser.find_element_by_xpath("//*[@id='input_agg_threshold']").clear()
                        self.browser.find_element_by_xpath("//*[@id='input_agg_threshold']").send_keys(agg_threshold)
                        log.info("设置告警阈值: {0}".format(agg_threshold))
                    # 点击保存
                    self.browser.find_element_by_xpath("//*[@id='result_agg_save']").click()
                else:
                    raise KeyError("标签【{0}】未加入到告警区域配置，请检查")

        # 配置预览
        if view_config == "是":
            view_config_button = self.browser.find_element_by_xpath("//*[@id='view_config_sql_text']")
            ...
            sleep(3)
            # 获取sql内容
            view_sql_area = self.browser.find_element_by_xpath("//*[@id='rule_sql_view']")
            view_sql = view_sql_area.get_attribute("value")
            log.info("获取sql预览内容: \n{0}".format(view_sql))

        # 规则测试
        if test_config == "是":
            test_button = self.browser.find_element_by_xpath("//*[@id='test_config_sql']")
    ...
    def result_config(self, result_field):
        ...
        if result_field:
            for field in result_field:
                ...
                # 根据字段名获取行号
                field_row = self.browser.find_element_by_xpath("//*[@data-content='{0}']/..".format(field))
                ...
                # 判断指定行的复选框是否勾选
                data_checkbox = self.browser.find_element_by_xpath(
                    "//*[@class='layui-table-body']//*[@data-index='{0}']/td/div/div".format(data_index))
                ...
                if not checked:
                    self.browser.find_element_by_xpath(
                        "//*[@class='layui-table-body']//*[@data-index='{0}']/td/div/div/i".format(data_index)).click()
    ...
    def storage_config(self, config_ftp, config_table):
        ...
        if config_ftp:
            # 获取开关状态
            ftp_open_button = self.browser.find_element_by_xpath(
                "//*[@id='rule_output_ftp_switch_id']/following-sibling::div")
            ...
            if config_ftp.__contains__("FTP名称"):
                ...
                self.browser.find_element_by_xpath("//*[@id='select_output_ftp_name']/following-sibling::div//i").click()
                ftp_name_ele = self.browser.find_element_by_xpath(
                    "//*[@id='select_output_ftp_name']/following-sibling::div[1]//dd[text()='{0}']".format(ftp_name))
            ...
            if config_ftp.__contains__("存储目录"):
                ...
                self.browser.find_element_by_xpath("//*[@id='select_output_ftp_save_directory']").clear()
                self.browser.find_element_by_xpath("//*[@id='select_output_ftp_save_directory']").send_keys(save_dir)
                log.info("设置存储目录: {0}".format(save_dir))

            # 文件名称
            if config_ftp.__contains__("文件名称"):
                ftp_file_name = config_ftp.get("文件名称")
                self.browser.find_element_by_xpath("//*[@id='select_output_ftp_file_name']").clear()
                self.browser.find_element_by_xpath("//*[@id='select_output_ftp_file_name']").send_keys(ftp_file_name)
            ...
            if config_ftp.__contains__("文件类型"):
                ...
                self.browser.find_element_by_xpath(
                    "//*[@id='select_output_ftp_file_type']/following-sibling::div//i").click()
                self.browser.find_element_by_xpath(
                    "//*[@id='select_output_ftp_file_type']/following-sibling::div[1]//dd[text()='{0}']".format(
                        ftp_file_type)).click()
            ...
            if config_ftp.__contains__("时间单位"):
                ...
                self.browser.find_element_by_xpath(
                    "//*[@id='select_output_ftp_save_time_unit']/following-sibling::div//i").click()
                self.browser.find_element_by_xpath(
                    "//*[@id='select_output_ftp_save_time_unit']/following-sibling::div[1]//dd[text()='{0}']".format(
                        time_unit)).click()
            ...
            if config_ftp.__contains__("保存间隔"):
                ...
                self.browser.find_element_by_xpath(
                    "//*[@id='select_output_ftp_save_interval']/following-sibling::div//i").click()
                interval_ele = self.browser.find_element_by_xpath(
                    "//*[@id='select_output_ftp_save_interval']/following-sibling::div[1]//dd[text()='{0}']".format(
                        interval))
            ...
            if config_ftp.__contains__("保存方式"):
                ...
                self.browser.find_element_by_xpath(
                    "//*[@id='select_output_ftp_save_type']/following-sibling::div//i").click()
                self.browser.find_element_by_xpath(
                    "//*[@id='select_output_ftp_save_type']/following-sibling::div[1]//dd[text()='{0}']".format(
                        save_type)).click()
        ...
        if config_table:
            # 获取开关状态
            table_open_button = self.browser.find_element_by_xpath(
                "//*[@id='rule_output_table_switch_id']/following-sibling::div")
            ...
            if config_table.__contains__("选择数据库"):
                ...
                self.browser.find_element_by_xpath(
                    "//*[@id='select_output_databbase']/following-sibling::div//i").click()
                database_ele = self.browser.find_element_by_xpath(
                    "//*[@id='select_output_databbase']/following-sibling::div[1]//dd[text()='{0}']".format(
                        database_name))
            ...
            if config_table.__contains__("选择表"):
                ...
                self.browser.find_element_by_xpath(
                    "//*[@id='select_output_database_table']/following-sibling::div//i").click()
                table_ele = self.browser.find_element_by_xpath(
                    "//*[@id='select_output_database_table']/following-sibling::div[1]//dd[text()='{0}']".format(
                        table_name))
            ...
            if config_table.__contains__("字段映射"):
                ...
                for field_rela in field_map:
                    ...
                    # 点击操作
                    self.browser.find_element_by_xpath(
                        "//*[text()='{0}']/../following-sibling::td[3]//a".format(field_name)).click()
                    ...
                    # 选择映射的输出字段
                    self.browser.find_element_by_xpath(
                        "//*[@id='output_field_radio_group']//*[contains(text(),'{0}')]".format(
                            output_field_name)).click()
                    sleep(1)
                    # 确认
                    self.browser.find_element_by_xpath("//*[text()='确认']").click()
    ...
    def update_status(self, obj, set_status, research=True):
        ...
        if set_status == "启用":
            if current_status:
                ...
                self.browser.find_element_by_xpath(
                    "//*[text()='{0}']/../../../following-sibling::td[2]/div/span".format(obj)).click()
                ...
                if alert.title_contains("确认启用{0}吗".format(obj), auto_click_ok=False):
                    ...
                    if alert.title_contains("操作成功"):
                        ...
                        log.warn("{0} 启用失败，失败提示: {1}".format(obj, msg))
                else:
                    log.warn("{0} 启用失败，失败提示: {1}".format(obj, msg))
                set_global_var("ResultMsg", msg, False)
        else:
            if current_status:
                ...
                if alert.title_contains("确认禁用{0}吗".format(obj), auto_click_ok=False):
                    ...
                    if alert.title_contains("操作成功"):
                        ...
                        log.warn("{0} 禁用失败，失败提示: {1}".format(obj, msg))
                else:
                    log.warn("{0} 禁用失败，失败提示: {1}".format(obj, msg))
                set_global_var("ResultMsg", msg, False)
            else:
    ...
    def query(self, query):
        ...
        if isinstance(query, dict):
            ...
            if query.__contains__("告警计划名称"):
                ...
                self.browser.find_element_by_xpath("//*[@name='alarmPlanName']/preceding-sibling::input").clear()
                self.browser.find_element_by_xpath(
                    "//*[@name='alarmPlanName']/preceding-sibling::input").send_keys(alarm_plan_name)
                log.info("告警计划名称输入: {0}".format(alarm_plan_name))

            # 告警规则名称
            if query.__contains__("告警规则名称"):
                alarm_rule_name = query.get("告警规则名称")
                self.browser.find_element_by_xpath("//*[@name='alarmRuleName']/preceding-sibling::input").clear()
                self.browser.find_element_by_xpath(
                    "//*[@name='alarmRuleName']/preceding-sibling::input").send_keys(alarm_rule_name)
            ...
            if query.__contains__("状态"):
                status = query.get("状态")
                self.browser.find_element_by_xpath(
                    "//*[@textboxname ='alarmStatus']/following-sibling::span//a").click()
                sleep(1)
                self.browser.find_element_by_xpath(
                    "//*[contains(@id,'_easyui_combobox') and text()='{0}']".format(status)).click()
                log.info("状态选择: {0}".format(status))

            # 创建人
            if query.__contains__("创建人"):
                creator = query.get("创建人")
                self.browser.find_element_by_xpath("//*[@name='creator']/preceding-sibling::input").clear()
                self.browser.find_element_by_xpath("//*[@name='creator']/preceding-sibling::input").send_keys(creator)
                log.info("创建人输入: {0}".format(creator))

            # 告警等级
            if query.__contains__("告警等级"):
                alarm_level = query.get("告警等级")
                self.browser.find_element_by_xpath("//*[@textboxname ='alarmLevelId']/following-sibling::span//a").click()
                sleep(1)
                alarm_level_element = self.browser.find_element_by_xpath(
                    "//*[contains(@id,'alarmLevelId') and text()='{0}']".format(alarm_level))
                action = ActionChains(self.browser)
            ...
            if query.__contains__("告警类型"):
                ...
                self.browser.find_element_by_xpath("//*[@textboxname ='alarmTypeId']/following-sibling::span//a").click()
                sleep(1)
                self.browser.find_element_by_xpath(
                    "//*[contains(@id,'alarmTypeId') and text()='{0}']".format(alarm_type)).click()
                log.info("告警类型选择: {0}".format(alarm_type))

            # 查询
            self.browser.find_element_by_xpath("//*[@funcid='AlarmPlatform_rule_query']//*[text()='查询']").click()
            page_wait()
            sleep(1)
            return
        else:
    ...
    def batch_enable(self, query):
        ...
        row_status = self.browser.find_elements_by_xpath(
            "//*[@funcid='AlarmPlatform_rule_active']/following-sibling::span")
        if len(row_status) > 0:
            num = 0
                ...
                if current_status:
                    ...
                    if alert.title_contains("确认启用", auto_click_ok=False):
                        ...
                        if alert.title_contains("操作成功"):
                            ...
                            log.warn("第{0}行启用失败，失败提示: {1}".format(num+1, msg))
                            break
                    else:
                        log.warn("第{0}行启用失败，失败提示: {1}".format(num+1, msg))
                        break
                ...
                if not current_status:
                    ...
                    if alert.title_contains("确认禁用", auto_click_ok=False):
                        ...
                        if alert.title_contains("操作成功"):
                            ...
                            log.warn("第{0}行禁用失败，失败提示: {1}".format(num+1, msg))
                            break
                    else:
                        log.warn("第{0}行禁用失败，失败提示: {1}".format(num+1, msg))
                        break
    ...
    def redo(self, obj, start_time=None, end_time=None):
        ...
        log.info("开始重调告警规则: {0}".format(obj))
        self.browser.switch_to.parent_frame()
        wait = WebDriverWait(self.browser, 10)
        wait.until(ec.frame_to_be_available_and_switch_to_it((
        ...
        if alert.title_contains("操作成功"):
            ...
            log.warn("{0} 重调失败，失败提示: {1}".format(obj, msg))
        set_global_var("ResultMsg", msg, False)
    ...
    def data_clear(self, obj, fuzzy_match=False):
        ...
        self.browser.find_element_by_xpath("//*[@name='alarmRuleName']/preceding-sibling::input[1]").clear()
        self.browser.find_element_by_xpath("//*[@name='alarmRuleName']/preceding-sibling::input[1]").send_keys(obj)
        self.browser.find_element_by_xpath("//*[text()='查询']").click()
        page_wait()
        if fuzzy_match:
            record_element = self.browser.find_elements_by_xpath(
                "//*[@field='alarmRuleName']/div/span[@class='tree-title']/a[starts-with(text(),'{0}')]".format(obj))
        else:
            record_element = self.browser.find_elements_by_xpath(
                "//*[@field='alarmRuleName']/div/span[@class='tree-title']/a[text()='{0}']".format(obj))
                ...
                if alert.title_contains("您确定需要删除", auto_click_ok=False):
                    ...
                    if alert.title_contains("成功"):
                        ...
                        if fuzzy_match:
                            # 重新获取页面查询结果
                            record_element = self.browser.find_elements_by_xpath(
                                "//*[@field='alarmRuleName']/div/span[@class='tree-title']/a[starts-with(text(),'{0}')]".format(
                                    obj))
                            if len(record_element) > 0:
                                exist_data = True
                            else:
