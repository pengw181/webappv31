...
class FTP:
    ...
    def choose(self, ftp_name):
        ...
        """
        input_ele = self.browser.find_element_by_xpath("//*[@id='ftpName']/following-sibling::span[1]/input[1]")
        input_ele.clear()
        input_ele.send_keys(ftp_name)
        self.browser.find_element_by_xpath("//span[text()='查询']").click()
        page_wait()
        self.browser.find_element_by_xpath("//*[@field='ftpName']//a[text()='{}']".format(ftp_name)).click()
    ...
    def add(self, ftp_name, host, port, username, password):
        ...
        if alert.title_contains("提交成功"):
            ...
            log.warn("数据 {0} 添加失败，失败提示: {1}".format(ftp_name, msg))
    ...
    def ftp_page(self, ftp_name, host, port, username, password):
        ...
        if ftp_name:
            self.browser.find_element_by_xpath(
                "//*[@id='editDiv']//*[@id='ftpName']/following-sibling::span/input[1]").clear()
            self.browser.find_element_by_xpath(
                "//*[@id='editDiv']//*[@id='ftpName']/following-sibling::span/input[1]").send_keys(ftp_name)
            log.info("设置FTP名称: {0}".format(ftp_name))

        # 连接地址
        if host:
            self.browser.find_element_by_xpath(
                "//*[@id='editDiv']//*[@id='ftpHostname']/following-sibling::span/input[1]").clear()
            self.browser.find_element_by_xpath(
                "//*[@id='editDiv']//*[@id='ftpHostname']/following-sibling::span/input[1]").send_keys(host)
            log.info("设置连接地址: {0}".format(host))

        # 端口
        if port:
            self.browser.find_element_by_xpath(
                "//*[@id='editDiv']//*[@id='ftpPort']/following-sibling::span/input[1]").clear()
            self.browser.find_element_by_xpath(
                "//*[@id='editDiv']//*[@id='ftpPort']/following-sibling::span/input[1]").send_keys(port)
            log.info("设置端口: {0}".format(port))

        # 用户名
        if username:
            self.browser.find_element_by_xpath(
                "//*[@id='editDiv']//*[@id='ftpUsername']/following-sibling::span/input[1]").clear()
            self.browser.find_element_by_xpath(
                "//*[@id='editDiv']//*[@id='ftpUsername']/following-sibling::span/input[1]").send_keys(username)
            log.info("设置用户名: {0}".format(username))

        # 密码
        if password:
            self.browser.find_element_by_xpath(
                "//*[@id='editDiv']//*[@id='ftpPassword']/following-sibling::span/input[1]").clear()
            self.browser.find_element_by_xpath(
                "//*[@id='editDiv']//*[@id='ftpPassword']/following-sibling::span/input[1]").send_keys(password)
            sleep(1)
    ...
    def test(self, ftp_name):
        ...
        if alert.title_contains("测试成功"):
            ...
            log.warn("{0} 测试失败，测试返回结果: {1}".format(ftp_name, msg))
        set_global_var("ResultMsg", msg, False)
    ...
    def data_clear(self, obj, fuzzy_match=False):
        ...
        self.browser.find_element_by_xpath("//*[@id='ftpName']/following-sibling::span[1]/input[1]").clear()
        self.browser.find_element_by_xpath(
            "//*[@id='ftpName']/following-sibling::span[1]/input[1]").send_keys(obj)
        self.browser.find_element_by_xpath("//*[@id='btn']//*[text()='查询']").click()
        page_wait()
        if fuzzy_match:
            record_element = self.browser.find_elements_by_xpath(
                "//*[@field='ftpName']/*[contains(@class,'ftpName')]/*[starts-with(text(),'{}')]".format(obj))
        else:
            record_element = self.browser.find_elements_by_xpath(
                "//*[@field='ftpName']/*[contains(@class,'ftpName')]/*[text()='{}']".format(obj))
                ...
                if alert.title_contains("您确定需要删除{0}吗".format(search_result), auto_click_ok=False):
                    ...
                    if alert.title_contains("成功"):
                        ...
                        if fuzzy_match:
                            # 重新获取页面查询结果
                            record_element = self.browser.find_elements_by_xpath(
                                "//*[@field='ftpName']/*[contains(@class,'ftpName')]/*[starts-with(text(),'{0}')]".format(
                                    obj))
                            if len(record_element) > 0:
                                exist_data = True
                            else:
