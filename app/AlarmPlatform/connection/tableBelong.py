...
class TableBelong:
    ...
    def choose(self, table_cn_name):
        ...
        """
        input_ele = self.browser.find_element_by_xpath("//*[@id='tableName']/following-sibling::span[1]/input[1]")
        input_ele.clear()
        input_ele.send_keys(table_cn_name)
        self.browser.find_element_by_xpath("//span[text()='查询']").click()
        page_wait()
        self.browser.find_element_by_xpath("//*[@field='tableNameCh']//a[text()='{}']".format(table_cn_name)).click()
    ...
    def add(self, database_name, table_en_ame, table_cn_Name, table_object, table_period, remark):
        ...
        if alert.title_contains("提交成功"):
            ...
            log.warn("数据 {0} 添加失败，失败提示: {1}".format(table_cn_Name, msg))
    ...
    def table_belong_page(self, database_name, table_en_ame, table_cn_Name, table_object, table_period, remark):
        ...
        if database_name:
            self.browser.find_element_by_xpath(
                "//*[@id='editDiv']//*[@id='databaseInfoId']/following-sibling::span//a").click()
            self.browser.find_element_by_xpath(
                "//*[contains(@id,'databaseInfoId') and text()='{0}']".format(database_name)).click()
            log.info("设置数据库: {0}".format(database_name))

        # 表英文名称
        if database_name:
            self.browser.find_element_by_xpath(
                "//*[@id='editDiv']//*[@id='tableNameEn']/following-sibling::span/input[1]").clear()
            self.browser.find_element_by_xpath(
                "//*[@id='editDiv']//*[@id='tableNameEn']/following-sibling::span/input[1]").send_keys(table_en_ame)
            log.info("设置表英文名称: {0}".format(table_en_ame))

        # 表中文名称
        if table_cn_Name:
            self.browser.find_element_by_xpath(
                "//*[@id='editDiv']//*[@id='tableNameCh']/following-sibling::span/input[1]").clear()
            self.browser.find_element_by_xpath(
                "//*[@id='editDiv']//*[@id='tableNameCh']/following-sibling::span/input[1]").send_keys(table_cn_Name)
            log.info("设置表中文名称: {0}".format(table_cn_Name))

        # 表使用对象
        if table_object:
            self.browser.find_element_by_xpath(
                "//*[@id='editDiv']//*[@id='tableObject']/following-sibling::span//a").click()
            self.browser.find_element_by_xpath(
                "//*[contains(@id,'tableObject') and text()='{0}']".format(table_object)).click()
            log.info("设置表使用对象: {0}".format(table_object))

        # 表周期
        if table_period:
            self.browser.find_element_by_xpath(
                "//*[@id='editDiv']//*[@id='tablePeriod']/following-sibling::span//a").click()
            self.browser.find_element_by_xpath(
                "//*[contains(@id,'tablePeriod') and text()='{0}']".format(table_period)).click()
    ...
    def data_clear(self, obj, fuzzy_match=False):
        ...
        self.browser.find_element_by_xpath("//*[@id='tableName']/following-sibling::span[1]/input[1]").clear()
        self.browser.find_element_by_xpath(
            "//*[@id='tableName']/following-sibling::span[1]/input[1]").send_keys(obj)
        self.browser.find_element_by_xpath("//*[@id='btn']//*[text()='查询']").click()
        page_wait()
        if fuzzy_match:
            record_element = self.browser.find_elements_by_xpath(
                "//*[@field='tableNameCh']/*[contains(@class,'tableNameCh')]/*[starts-with(text(),'{}')]".format(obj))
        else:
            record_element = self.browser.find_elements_by_xpath(
                "//*[@field='tableNameCh']/*[contains(@class,'tableNameCh')]/*[text()='{}']".format(obj))
                ...
                if alert.title_contains("您确定需要删除{0}吗".format(search_result), auto_click_ok=False):
                    ...
                    if alert.title_contains("成功"):
                        ...
                        if fuzzy_match:
                            # 重新获取页面查询结果
                            record_element = self.browser.find_elements_by_xpath(
                                "//*[@field='tableNameCh']/*[contains(@class,'tableNameCh')]/*[starts-with(text(),'{0}')]".format(
                                    obj))
                            if len(record_element) > 0:
                                exist_data = True
                            else:
