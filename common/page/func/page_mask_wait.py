...
def page_wait(interval=0.1, timeout=30):
    ...
    while time_count <= timeout:
        try:
            browser.find_element_by_xpath("//*[@class='datagrid-mask' and @style='display:block']")
            time_count += interval
            sleep(interval)
            continue
        ...
        try:
            browser.find_element_by_xpath("//*[contains(@class,'datagrid-mask') and contains(@style,'display: block')]")
            time_count += interval
            sleep(interval)
            continue
        except NoSuchElementException:
        ...
        try:
            browser.find_element_by_xpath("//*[@id='loadingDiv']")
            time_count += interval
            sleep(interval)
            continue
        except NoSuchElementException:
