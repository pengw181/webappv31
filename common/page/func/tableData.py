...
def get_table_data(data_table_xpath, column_table_xpath=None, limit=20, return_column=False):
    ...
    if column_table_xpath:
        column_field = browser.find_elements_by_xpath(column_table_xpath + "/tbody/tr/td/div/span[1]")
    ...
    for i in range(1, limit+1):
        try:
            data_grid = browser.find_elements_by_xpath(data_table_xpath + "//tr[{}]//div".format(i))
            tmp = []
...
def get_table_data2(table_xpath, return_column=False):
    ...
    # 获取列名信息
    if return_column:
        column_field = browser.find_elements_by_xpath(table_xpath + "/thead/tr/th")
        for c in column_field:
            columns.append(c.get_attribute("innerText"))
        log.info("表格列信息:【{}】".format(",".join(columns)))
    ...
    i = 1
    while flag:
        data_grid = browser.find_elements_by_xpath(table_xpath + "/tbody/tr[{}]/td".format(i))
        if len(data_grid) == 0:
            flag = False
        else:
            tmp = []
