...
def regular_cube(set_type="添加", regular_name=None, advance_mode=None, regular=None, expression=None, enable_check=None,
                 check_msg=None, confirm_selector="//*[@id='regexDiv']"):
    """
    # 用来操作正则魔方
...
def set_label_attribute(browser, confirm_selector, label_name, tag_info):
    """
    # 设置标签属性
    :param browser: 浏览器
    :param confirm_selector: 用来定位，推荐用id定位到父节点tr或div标签
...
def analyze_conf(begin_row=None, enable_magic=None, total_columns=None, row_split_type=None, split_tag=None, magic=None,
                 advance_conf=None, confirm_selector="//*[@id='regexptableFormatCfgDiv']"):
    """
    ...
    # 正则魔方
    if magic:
        regular_cube(set_type=magic.get("设置方式"), regular_name=magic.get("正则模版名称"),
                     advance_mode=magic.get("高级模式"), regular=magic.get("标签配置"), expression=magic.get("表达式"),
                     confirm_selector=confirm_selector)

    # 如果是选择正则魔方，analyze_add_flag为false,上面会自动返回到上层iframe，调用方只需要手动关闭窗口或不做处理。
    # 如果analyze_add_flag为true，通知调用方手动跳转iframe。
    if not magic:
        # 如果不需要配置正则魔方，则返回false
        analyze_add_flag = False
    else:
        if magic.get("设置方式") == "添加":
            # 如果需要配置正则魔方，且设置方式为添加，则返回true
            analyze_add_flag = True
        else:
            # 如果需要配置正则魔方，且设置方式为选择，则返回false
            analyze_add_flag = False
    return analyze_add_flag
