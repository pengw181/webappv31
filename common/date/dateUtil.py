...
def set_calendar(date_s, date_format="%Y-%m-%d"):
    ...
    # 设置时分秒
    hms_ele = browser.find_elements_by_xpath("//*[contains(@class,'timespinner-f')]/following-sibling::span//input[1]")
    for hms in hms_ele:
        if hms.is_displayed():
            hms.clear()
            if second:
            ...
            # 点击确定
            ok_button = browser.find_elements_by_xpath("//*[@class='datebox-button']//*[text()='确定']")
            for ok in ok_button:
                if ok.is_displayed():
                    ok.click()
                    break
...
def set_laydate(date_s, date_format="%Y-%m-%d"):
    ...
    if date_s == "now":
        # 现在
        browser.find_element_by_xpath("//*[@lay-type='now']").click()
    else:
        # date_s = "2019-02-27 17:50:30"
        time = datetime.strptime(date_s, date_format)
        ...
        # 设置年
        browser.find_element_by_xpath("//*[@class='laydate-set-ym']/span[1]").click()
        try:
            browser.find_element_by_xpath("//*[@lay-ym='{0}']".format(year)).click()
            year_is_show = True
        except NoSuchElementException:
            year_is_show = False
        while year_is_show is False:
            # 判断年是否在可选范围内
            year_period_obj = browser.find_element_by_xpath("//*[@class='laydate-set-ym']")
            ...
            if year < int(year_period_begin):
                # 如果年比当前最小年还小，点击往前翻页
                browser.find_element_by_xpath("//*[contains(@class,'laydate-prev-y')]").click()
            elif year > int(year_period_end):
                # 如果年比当前最大年还大，点击往后翻页
                browser.find_element_by_xpath("//*[contains(@class,'laydate-next-y')]").click()
            else:
                browser.find_element_by_xpath("//*[@lay-ym='{0}']".format(year)).click()
                year_is_show = True
        ...
        browser.find_element_by_xpath("//*[@class='laydate-set-ym']/span[2]").click()
        browser.find_element_by_xpath("//*[@lay-ym='{0}']".format(show_month)).click()
        ...
        browser.find_element_by_xpath("//*[@lay-ymd='{0}']".format(ymd)).click()
        # log.info("选择日: {0}".format(day))

        # 点开选择时间
        if hour is not None:
            browser.find_element_by_xpath("//*[text()='选择时间']").click()
        ...
        if hour is not None:
            if hour < 10:
                hour = "0" + str(hour)
            hour_ele = browser.find_element_by_xpath(
                "//*[text()='时']/following-sibling::ol/li[text()='{0}']".format(hour))
        ...
        if minute is not None:
            if minute < 10:
                minute = "0" + str(minute)
            minute_ele = browser.find_element_by_xpath(
                "//*[text()='分']/following-sibling::ol/li[text()='{0}']".format(minute))
        ...
        if second is not None:
            if second < 10:
                second = "0" + str(second)
            second_ele = browser.find_element_by_xpath(
                "//*[text()='秒']/following-sibling::ol/li[text()='{0}']".format(second))
            action.move_to_element(second_ele).click().perform()
            second_ele.click()
            # log.info("设置秒: {0}".format(second))

        # 点击确定
        browser.find_element_by_xpath("//*[@lay-type='confirm']").click()
